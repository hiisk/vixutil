...
const APP = appJoin('food');

/**
 * 카탈로그(lib/food-tools.ts) 하나를 허브·상세 셸·검색 인덱스·사이트맵·FAQ가
 * 나눠 읽는다. 도구를 늘리다가 한 곳을 빠뜨리면 화면에는 보이는데 검색에서
 * 안 나오거나 색인이 안 된다.
 */

test('열 개 이상의 도구를 담는다', () => {
  assert.ok(FOOD_TOOLS.length >= 10, `도구가 ${FOOD_TOOLS.length}개뿐`);
  assert.ok(new Set(FOOD_TOOLS.map(t => t.category)).size >= 3, '분류가 너무 적다');
});
...
test('도구마다 페이지와 OG 이미지가 있다', () => {
  ...
  for (const t of FOOD_TOOLS) {
    ...
    if (!hasOwnCard(`/food/${t.slug}`)) missing.push(`/food/${t.slug} 공유 카드`);
  }
...
test('페이지 폴더마다 카탈로그 항목이 있다', () => {
  const orphans = readdirSync(APP, { withFileTypes: true })
    ...
    // [slug]는 재료 125가지를 그리는 동적 라우트다 — 도구 카탈로그가 아니라
    // lib/food/ingredients8.ts에서 목록이 오므로 여기서 셀 대상이 아니다
    .filter(e => !e.name.startsWith('['))
    .map(e => e.name)
    .filter(name => !findFoodTool(name));
...
test('slug가 중복되지 않는다', () => {
  const slugs = FOOD_TOOLS.map(t => t.slug);
...
test('제목과 설명이 서로 겹치지 않는다', () => {
  for (const key of ['metaTitle', 'long'] as const) {
    const vals = FOOD_TOOLS.map(t => t[key]);
...
test('상세 페이지는 공용 셸을 쓴다', () => {
  ...
  for (const t of FOOD_TOOLS) {
    ...
    if (!src.includes('FoodShell')) bad.push(`${t.slug}: 셸 미사용`);
    if (!src.includes(`slug="${t.slug}"`)) bad.push(`${t.slug}: 셸에 넘긴 slug가 다르다`);
    if (!src.includes(`canonical: '/food/${t.slug}'`)) bad.push(`${t.slug}: canonical 없음`);
...
test('모든 도구에 FAQ가 있다', () => {
  const missing = FOOD_TOOLS.filter(t => (SECTION_FAQ[`food/${t.slug}`] ?? []).length < 2).map(t => t.slug);
  assert.deepEqual(missing, [], `FAQ가 2개 미만인 도구: ${missing.join(', ')}`);
  assert.ok((SECTION_FAQ.food ?? []).length >= 3, '허브 FAQ가 부족하다');
});
...
test('허브가 모든 도구를 링크한다', () => {
  ...
  const orphan = FOOD_TOOLS.filter(t => !known.has(t.category)).map(t => `${t.slug}(${t.category})`);
...
test('검색 인덱스·사이트맵·푸터가 이 섹션을 싣는다', () => {
  ...
  assert.ok(index.includes('FOOD_TOOLS'), '검색 인덱스가 이 섹션을 싣지 않는다');
  assert.ok(index.includes('food:'), 'SECTION_META에 food가 없다');

  const sitemap = readFileSync(appJoin('sitemap.ts'), 'utf8');
  assert.ok(sitemap.includes('/food'), '사이트맵에 허브가 없다');
  assert.ok(sitemap.includes('FOOD_TOOLS'), '사이트맵에 상세 페이지가 없다');

  const footer = readFileSync(join(ROOT, 'components', 'SiteFooter.tsx'), 'utf8');
  assert.ok(footer.includes('"/food"'), '푸터에 이 섹션이 없다');
});

test('관련 도구 추천이 자기 자신을 넣지 않는다', () => {
  for (const t of FOOD_TOOLS) {
    const related = relatedFoodTools(t.slug);
...
test('없는 slug로는 도구를 찾지 못한다', () => {
  assert.equal(findFoodTool('없는도구'), undefined);
  assert.deepEqual(relatedFoodTools('없는도구'), []);
});
