...
test('열 개 이상의 도구를 담는다', () => {
  assert.ok(SOUND_TOOLS.length >= 10, `도구가 ${SOUND_TOOLS.length}개뿐`);
  assert.ok(new Set(SOUND_TOOLS.map(t => t.category)).size >= 3, '분류가 너무 적다');
});

test('도구마다 페이지와 OG 이미지가 있다', () => {
  const missing: string[] = [];
  for (const t of SOUND_TOOLS) {
...
test('페이지 폴더마다 카탈로그 항목이 있다', () => {
  const orphans = readdirSync(APP, { withFileTypes: true })
    ...
    // hz는 도구가 아니라 주파수 113가지를 그리는 자료 갈래다 —
    // 목록이 lib/sound/freqs.ts에서 오므로 도구 카탈로그가 셀 대상이 아니다
    .filter(e => e.name !== 'hz')
    .map(e => e.name)
    .filter(name => !findSoundTool(name));
  assert.deepEqual(orphans, [], `카탈로그에 없는 페이지 폴더: ${orphans.join(', ')}`);
});

test('주파수 라우트는 주파수 목록에서 페이지를 만든다', () => {
  // 위 검사에서 hz를 빼 주었으니, 그 라우트가 실제로 주파수 목록을 쓰는지 여기서 본다
  const src = readFileSync(join(APP, 'hz', '[slug]', 'page.tsx'), 'utf8');
  assert.ok(src.includes('freqParams'), '[slug] 라우트가 주파수 목록을 돌지 않는다');
  assert.ok(src.includes('generateStaticParams'), '[slug] 라우트가 정적 경로를 만들지 않는다');
  assert.ok(existsSync(join(APP, 'hz', 'page.tsx')), '주파수 목록 페이지가 없다');
  assert.ok(existsSync(join(APP, 'hz', '[slug]', 'opengraph-image.tsx')), '공유 카드가 없다');
});

test('slug가 중복되지 않는다', () => {
  const slugs = SOUND_TOOLS.map(t => t.slug);
...
test('제목과 설명이 서로 겹치지 않는다', () => {
  for (const key of ['metaTitle', 'long'] as const) {
    const vals = SOUND_TOOLS.map(t => t[key]);
...
test('상세 페이지는 공용 셸을 쓴다', () => {
  ...
  for (const t of SOUND_TOOLS) {
    ...
    if (!src.includes('SoundShell')) bad.push(`${t.slug}: 셸 미사용`);
    if (!src.includes(`slug="${t.slug}"`)) bad.push(`${t.slug}: 셸에 넘긴 slug가 다르다`);
    if (!src.includes(`canonical: '/sound/${t.slug}'`)) bad.push(`${t.slug}: canonical 없음`);
...
test('모든 도구에 FAQ가 있다', () => {
  const missing = SOUND_TOOLS.filter(t => (SECTION_FAQ[`sound/${t.slug}`] ?? []).length < 2).map(t => t.slug);
  assert.deepEqual(missing, [], `FAQ가 2개 미만인 도구: ${missing.join(', ')}`);
  assert.ok((SECTION_FAQ.sound ?? []).length >= 3, '허브 FAQ가 부족하다');
});
...
test('허브가 모든 도구를 링크한다', () => {
  ...
  const orphan = SOUND_TOOLS.filter(t => !known.has(t.category)).map(t => `${t.slug}(${t.category})`);
...
test('검색 인덱스·사이트맵·푸터가 이 섹션을 싣는다', () => {
  ...
  assert.ok(footer.includes('"/sound"'), '푸터에 이 섹션이 없다');
});

test('관련 도구 추천이 자기 자신을 넣지 않는다', () => {
  for (const t of SOUND_TOOLS) {
    const related = relatedSoundTools(t.slug);
...
test('없는 slug로는 도구를 찾지 못한다', () => {
  assert.equal(findSoundTool('없는도구'), undefined);
  assert.deepEqual(relatedSoundTools('없는도구'), []);
});
