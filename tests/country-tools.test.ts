...
test('세 언어의 이름·수도·언어·통화·소개·팁·비자가 모두 채워져 있다', () => {
  for (const c of COUNTRIES) {
...
test('나라 이름은 언어별로 겹치지 않는다', () => {
  for (const lang of LANGS) {
    const names = COUNTRIES.map(c => c[lang].name);
...
test('지역 이름이 세 언어로 다 있다', () => {
  for (const lang of LANGS) {
    for (const r of COUNTRY_REGIONS) assert.ok(COUNTRY_REGION_LABEL[lang][r], `${lang}에 ${r} 라벨 없음`);
  }
});
...
test('FAQ는 3개이고 실제 데이터가 들어간다', () => {
  for (const c of COUNTRIES) {
    for (const lang of LANGS) {
      ...
      assert.equal(faq[2].a, c[lang].visa);
      if (lang !== 'ko') {
...
test('비자 정책이 바뀔 수 있다는 안내가 세 언어에 모두 있다', () => {
  for (const lang of LANGS) {
