...
test('사이트맵의 국제 주소가 전부 받는 라우트를 가진다', { skip: sitemapRoutes() ? false : '빌드 산출물 없음' }, () => {
  ...
  /*
   * 문턱을 100,000 → 40,000으로 내렸다 (2026-08-20).
   *
   * 실측 49,371개다. 줄어든 까닭은 «깨져서»가 아니라 지워서다 — 8월 15일에
   * 양산형 값 격자 55개 갈래, 8월 18일에 country를 비롯한 7개 갈래를 통째로
   * 지웠다. 그런데 이 검사는 **빌드 산출물이 있어야 돌아서** 그동안 조용히
   * 건너뛰어졌고, 문턱만 옛 규모(16만 장 시절)로 남아 있었다.
   *
   * 이 수의 목적은 «규모가 맞나»가 아니라 «세는 방식이 통째로 깨졌나»다.
   * 그래서 실측보다 넉넉히 아래에 둔다.
   */
  assert.ok(checked > 40_000, `국제 주소를 ${checked}개밖에 못 봤다 — 사이트맵이나 세는 방식이 깨졌다`);
  assert.deepEqual(
...
test('낱장 라우트가 아홉 언어에 똑같이 있다', () => {
  ...
  /* 2026-08-15: 검색 수요가 없는 참조표 갈래를 통째로 지우면서 68 → 35로 내렸다
     2026-08-18: 조합 격자 낱장과 갈래 일곱을 지워 19다 */
  assert.ok(ref.slug.size > 16, `en 낱장 무늬가 ${ref.slug.size}개뿐 — 세는 방식이 깨졌다`);
  for (const lang of FOLD_LANGS) {
