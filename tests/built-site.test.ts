 ...
 * 검사에서 빼는 경로.
 *
 * ① Next가 만드는 내부 페이지 — 주소로 노출되지 않는다.
 *
 * ② **ISR이 요청 때 만들어 넣은 것.** 2026-08-13에 낱장을 ISR로 되돌린 뒤
 *    새로 생긴 문제다. 서버를 한 번 띄우면 들어온 요청마다 `.next/server/app`에
 *    페이지가 **파일로 쌓인다** — 없는 주소를 찔러도 404 페이지가 캐시된다.
 *    이 검사는 그 폴더를 읽으므로, 서버를 띄운 뒤 검사를 돌리면 내가 찌른
 *    쓰레기가 검사 대상이 되어 깨진다. 실제로 두 번 그렇게 깨졌다 —
 *    지어낸 슬러그 하나와, 헤드리스 크롬이 요청한 `/_vercel/insights/script.js`다.
 *
 *    `_`로 시작하는 칸이 있는 경로는 우리 페이지가 아니다(플랫폼·프레임워크
 *    자리다). 그것을 빼면 이 오염에 흔들리지 않는다. 그래도 남는 원칙은
 *    **빌드 검사는 서버를 띄우기 전에 돌린다**는 것이다.
 */
const INTERNAL = /(^|\/)_/;
