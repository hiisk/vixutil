 ...
 * 읽어야만 드러나는 종류의 문제다.
 *
 * 빌드 산출물이 없으면 건너뛴다 — npm run build 뒤에만 의미가 있다.
 *
 * 전에는 out/을 봤다. ISR로 바꾸면서 out/이 없어졌는데 조건을 안 고쳐서
 * **이 검사가 한동안 말없이 건너뛰었다.** 지금은 .next/server/app을 본다.
 * 허브는 빌드가 반드시 굽는 자리라 그대로 다 걸린다.
 */
const built = isBuilt();
...
function pageFor(locale: (typeof ALL_LOCALES)[number], route: string): string | null {
  return builtHtml(localeHref(locale, route));
}
...
test('canonical이 자기 주소를 가리킨다', { skip: built ? false : '빌드 산출물 없음 — npm run build 필요' }, () => {
  ...
  for (const sec of SECTIONS) {
    for (const locale of ALL_LOCALES) {
      ...
      if (canon !== want) wrong.push(`${localeHref(locale, `/${sec}`)}: ${canon} (기대 ${want})`);
    }
  }
  assert.deepEqual(wrong.slice(0, 10), [], `canonical이 틀린 페이지 ${wrong.length}장`);
});

test('번역 페이지가 자기 언어의 og:locale을 낸다', { skip: built ? false : '빌드 산출물 없음 — npm run build 필요' }, () => {
  /*
  ...
  */
  ...
  for (const sec of SECTIONS) {
    for (const locale of ALL_LOCALES) {
      ...
      if (og !== OG[locale]) wrong.push(`${localeHref(locale, `/${sec}`)}: ${og} (기대 ${OG[locale]})`);
    }
  }
