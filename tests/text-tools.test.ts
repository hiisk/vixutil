...
const APP = appJoin('text');

/**
 * 기기 점검·이미지 도구와 같은 구조다 — 카탈로그(lib/text-tools.ts) 하나를
 * 허브·상세 셸·검색 인덱스·사이트맵·FAQ가 나눠 읽는다. 도구를 늘리다가 한 곳을
 * 빠뜨리면 화면에는 보이는데 검색에서 안 나오거나 색인이 안 된다.
 */

test('열 개 이상의 도구를 담는다', () => {
  // 섹션 하나가 열 개 이상의 주제를 담는다는 기준이다. 그보다 적으면
  // 검색으로 들어온 사람이 옆 도구로 옮겨 다닐 이유가 없어진다.
  assert.ok(TEXT_TOOLS.length >= 10, `도구가 ${TEXT_TOOLS.length}개뿐`);
  assert.ok(new Set(TEXT_TOOLS.map(t => t.category)).size >= 3, '분류가 너무 적다');
});
...
test('도구마다 페이지와 OG 이미지가 있다', () => {
  ...
  for (const t of TEXT_TOOLS) {
    ...
    if (!hasOwnCard(`/text/${t.slug}`)) missing.push(`/text/${t.slug} 공유 카드`);
  }
...
test('페이지 폴더마다 카탈로그 항목이 있다', () => {
  const orphans = readdirSync(APP, { withFileTypes: true })
    ...
    // char와 regex는 도구가 아니라 자료 갈래다 — 특수문자 168자와 정규식 133가지를
    // 각각 lib/glyph/list.ts, lib/regex/list.ts에서 받아 그리므로 도구 카탈로그가 셀 대상이 아니다
    .filter(e => e.name !== 'char' && e.name !== 'regex')
    .map(e => e.name)
...
test('slug가 중복되지 않는다', () => {
  const slugs = TEXT_TOOLS.map(t => t.slug);
...
test('제목과 설명이 서로 겹치지 않는다', () => {
  for (const key of ['metaTitle', 'long'] as const) {
    const vals = TEXT_TOOLS.map(t => t[key]);
...
test('상세 페이지는 공용 셸을 쓴다', () => {
  ...
  for (const t of TEXT_TOOLS) {
    ...
    if (!src.includes('TextShell')) bad.push(`${t.slug}: 셸 미사용`);
    if (!src.includes(`slug="${t.slug}"`)) bad.push(`${t.slug}: 셸에 넘긴 slug가 다르다`);
    if (!src.includes(`canonical: '/text/${t.slug}'`)) bad.push(`${t.slug}: canonical 없음`);
...
test('모든 도구에 FAQ가 있다', () => {
  const missing = TEXT_TOOLS.filter(t => (SECTION_FAQ[`text/${t.slug}`] ?? []).length < 2).map(t => t.slug);
  assert.deepEqual(missing, [], `FAQ가 2개 미만인 도구: ${missing.join(', ')}`);
  assert.ok((SECTION_FAQ.text ?? []).length >= 3, '허브 FAQ가 부족하다');
});
...
test('허브가 모든 도구를 링크한다', () => {
  ...
  const orphan = TEXT_TOOLS.filter(t => !known.has(t.category)).map(t => `${t.slug}(${t.category})`);
...
test('검색 인덱스·사이트맵·푸터가 이 섹션을 싣는다', () => {
  ...
  assert.ok(index.includes('TEXT_TOOLS'), '검색 인덱스가 텍스트 도구를 싣지 않는다');
  assert.ok(index.includes('text:'), 'SECTION_META에 text가 없다');

  const sitemap = readFileSync(appJoin('sitemap.ts'), 'utf8');
  assert.ok(sitemap.includes('/text'), '사이트맵에 허브가 없다');
  assert.ok(sitemap.includes('TEXT_TOOLS'), '사이트맵에 상세 페이지가 없다');

  const footer = footerSrc();
  assert.ok(footer.includes('"/text"'), '푸터에 텍스트 도구가 없다');
});

test('도구 컴포넌트가 입력한 글을 밖으로 내보내지 않는다', () => {
  // 이 섹션의 약속은 "입력한 글이 브라우저를 벗어나지 않는다"이다. 계약서·자소서를
  // 붙여 넣으라고 안내하는 만큼, 전송 경로가 생기면 그 안내가 거짓이 된다.
  const dir = join(ROOT, 'components', 'text');
  ...
  assert.deepEqual(offenders, [], `입력한 글을 전송할 수 있는 코드가 있다: ${offenders.join(', ')}`);
});

test('관련 도구 추천이 자기 자신을 넣지 않는다', () => {
  for (const t of TEXT_TOOLS) {
    const related = relatedTextTools(t.slug);
...
test('같은 카테고리를 먼저 추천한다', () => {
  const first = relatedTextTools('hanyoung');
  assert.equal(first[0].category, '한글 변환', `첫 추천이 ${first[0].category}`);
});

test('없는 slug로는 도구를 찾지 못한다', () => {
  assert.equal(findTextTool('없는도구'), undefined);
  assert.deepEqual(relatedTextTools('없는도구'), []);
});
