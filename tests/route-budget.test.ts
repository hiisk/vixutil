 ...
 *                         하나(+1)를 되돌린다.
 *
 *   동적 430 / 정적 464   2026-08-15 — **빌드로 재니 430이었다. 541은 잰 값이
 *                         아니었다.**
 *
 *                         위 세 줄(673 → 532 → 541)은 전부 뺄셈과 덧셈으로 적은
 *                         값이다 — 갈래 하나가 동적 아홉을 문다고 보고 곱해서
 *                         뺐다. 그 셈이 실제 라우트 표와 어긋나 있었고, 아무도
 *                         빌드를 돌려 맞춰 보지 않아 111칸이 장부에만 남아 있었다.
 *                         이 검사가 `.next/routes-manifest.json`을 읽는데도
 *                         **빌드 없이는 skip이라** 그동안 안 걸렸다.
 *
 *                         **페이지는 한 장도 안 잃었다.** 확인한 방법:
 *                           · 사이트맵 286,841장 — 지우기 전 수와 같다
 *                           · 언어×갈래 조합 **19,877개를 전수로 찍어 전부 200**
 *                             (next start, 조합마다 실제 주소 하나씩)
 *                         줄어든 111칸은 낱장이 제 [slug] 라우트 대신 캐치올과
 *                         [a]/[b]로 들어간 자리다. 표에 여유가 그만큼 는 것이라
 *                         잃은 것이 아니라 번 것이다 — 2,048칸 중 지금 430이다.
 *
