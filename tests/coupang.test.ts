...
/*
 * ── 반복문 안의 광고는 여기서 못 잡는다 (2026-08-20) ────────
 * .map 안에 <Ad>가 들어가면 목록 개수만큼 광고가 뜬다. /hanja에서 실제로
 * **여섯 개**가 떴다 — 분류 격자 안쪽에 꽂혀서였다. 아래 «파일당 한 번» 검사는
 * 소스에 한 번만 적혀 있으니 통과한다.
 *
 * 소스만 보고 JSX 중첩을 판정하려고 두 가지를 해 봤고 둘 다 버렸다:
 *   · 괄호 깊이 세기      — 다 지난 map의 닫는 괄호까지 세어 멀쩡한 다섯을 걸었다
 *   · 앞선 .map과 key= 사이 — 파일 앞쪽의 무관한 map을 찾아 멀쩡한 열여덟을 걸었다
 *
 * 파서 없이는 안 된다. 멀쩡한 것을 걸어 대는 검사는 없느니만 못하다 — 곧
 * 무시당하고 우회된다. 이건 브라우저에서 세야 한다:
 *
 *     화면마다 document.querySelectorAll('aside')에서 「쿠팡 파트너스」를 담은
 *     것의 개수 == 1
 *
 * 광고 자리를 옮길 때마다 그 훑기를 돌린다.
 */
