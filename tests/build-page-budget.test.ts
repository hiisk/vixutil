...
test('빌드에서 구울 장수가 한도 안에 든다', () => {
  ...
  /* 2026-08-15: 검색 수요가 없는 참조표 갈래를 통째로 지우면서 592 → 380으로 내렸다 */
  /* 2026-08-18: 조합 격자 낱장 열다섯 갈래를 지워 267이다 */
  assert.ok(dynamic > 250, `동적 라우트가 ${dynamic}개뿐 — 세는 방식이 깨졌다`);
  assert.ok(staticN > 300, `정적 라우트가 ${staticN}개뿐 — 세는 방식이 깨졌다`);
  /* 2026-08-15: 찾아올 사람이 없는 갈래 스물여섯을 지워 등록부가 그만큼 짧아졌다
     (허브 하나가 아홉 장이므로 −234). 2,000 → 1,700으로 내린다 — 셈이 깨진 것과
     일부러 줄인 것을 가리려면 문턱을 낮추되 0으로 두지 않는다. */
  assert.ok(foldHubs > 1_700, `접힌 허브가 ${foldHubs}장뿐 — 등록부를 못 읽었다`);
