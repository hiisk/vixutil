...
const SHARED_CARD = [
  ...
  /\[/,
  /*
   * 정책·소개 네 장(2026-08-12) — 소개·문의·개인정보 처리방침·이용약관.
   * 계산기 낱장과 같은 이유다. SNS로 공유되는 물건이 아니고, 공유될 때 보여야
   * 하는 것은 "vixutil이라는 사이트"이므로 첫 화면 카드를 물려받는 것이 맞다.
   * 카드가 아예 안 붙는 것(canonical을 빼먹으면 그렇게 된다)과는 다르므로,
   * 첫 화면 카드가 실제로 붙는지는 tests/legal-pages.test.ts가 따로 본다.
   * 앞머리 자리는 언어다 — 열 언어에 네 장씩 있어서 'about'과 'zh-hant/about'이
   * 함께 온다(한국어는 앞머리가 없다).
   */
  /^(?:[a-z]{2}(?:-[a-z]+)?\/)?(?:about|contact|privacy|terms)$/,
];
