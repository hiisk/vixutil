...
test('자르는 자리를 바꿔도 주소가 늘거나 줄지 않는다', { skip }, () => {
  ...
  /*
   * 150,000 → 45,000으로 내렸다 (2026-08-20). 실측 56,928개다.
   *
   * 16만 장 시절의 문턱이 그대로 남아 있었다. 8월 15일에 양산형 값 격자 55개
   * 갈래, 8월 18일에 갈래 일곱을 지우면서 실제 규모가 3분의 1로 줄었는데,
   * 이 검사는 빌드 산출물이 있어야 돌아 그동안 조용히 건너뛰어졌다.
   *
   * 이 수가 지키려는 것은 규모가 아니라 «세는 방식이 통째로 깨졌나»다.
   */
  assert.ok(flat.length > 45_000, `주소가 ${flat.length}개뿐 — 세는 방식이 깨졌다`);
