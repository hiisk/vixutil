      ...
      <div className="flex flex-col gap-4">
        ...
        <Card className="p-5">
          <div className="flex justify-between items-center mb-3">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">추가 항목</p>
            <button
              type="button"
              onClick={addExtra}
          ...
          {extras.length === 0 && (
            <p className="text-sm text-slate-400 text-center py-4">추가 항목 없음 (음료, 주차비 등)</p>
          )}
          <div className="flex flex-col gap-3">
            {extras.map(ex => (
              <div key={ex.id} className="bg-slate-50 rounded-xl p-3 flex flex-col gap-2">
                ...
                <div className="flex items-end gap-2">
                  ...
                  <button
                    ...
                    className="py-3 px-3 text-sm text-red-400 border border-red-200 rounded-xl hover:bg-red-50 transition-colors"
                  >
                    삭제
                  </button>
                </div>
                ...
                <tbody>
                  {result.map((r, i) => (
                    <tr key={i}>
                      ...
                      <td>{w(r.base)}원</td>
                      <td className={r.extra > 0 ? 'text-orange-600 font-semibold' : 'text-slate-400'}>
                        {r.extra > 0 ? `+${w(r.extra)}원` : '-'}
                      </td>
                      <td className="font-black text-slate-900">{w(r.total)}원</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </TableWrap>
            <div className="px-5 py-3 bg-slate-50 rounded-b-2xl flex justify-between items-center">
              <span className="text-xs text-slate-500">총합 확인</span>
