              ...
              <div className="grid grid-cols-2 gap-2">
                {[{ v: 'house', l: '주택' }, { v: 'other', l: '비주택 (상가·토지)' }].map(o => (
                  <button key={o.v} onClick={() => setType(o.v as 'house' | 'other')}
                    className={`py-2.5 rounded-xl text-sm font-semibold transition-colors border ${type === o.v ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-600 border-slate-200 hover:border-blue-300'}`}>
                    {o.l}
                  </button>
                ))}
              </div>
            ...
            {type === 'house' && (
              <>
                ...
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  ...
                  <span className="text-sm text-slate-700">조정대상지역</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  <input type="checkbox" checked={firstBuy} onChange={e => setFirstBuy(e.target.checked)}
                    className="w-4 h-4 accent-blue-600" />
                  <span className="text-sm text-slate-700">생애최초 취득 (최대 200만원 감면)</span>
                </label>
              </>
            )}
