        ...
        <Card className="p-5">
          <div className="relative">
            <input
              ...
              className="w-full bg-white border border-slate-200 rounded-xl px-4 py-4 text-2xl font-bold text-slate-900 placeholder:text-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 pr-16"
            />
          ...
          <>
            ...
            <Card>
              <div className="px-5 py-3 border-b border-slate-100">
                <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">아파트 평형 참고표</p>
              </div>
              <div className="divide-y divide-slate-100">
                {REFS.map(r => (
                  <button
                    ...
                    className={`w-full px-5 py-3 flex justify-between items-center text-sm hover:bg-slate-50 transition-colors ${nearest?.label === r.label ? 'bg-blue-50' : ''}`}
                  >
                    <span className={`font-semibold ${nearest?.label === r.label ? 'text-blue-700' : 'text-slate-700'}`}>{r.label}</span>
                    <span className="text-slate-400">{r.pyeong}평 = {r.sqm}㎡</span>
                  </button>
                ))}
              </div>
            </Card>
