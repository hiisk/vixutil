    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="bg-white/90 backdrop-blur border-b border-slate-100 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-3">
          ...
          <span className="text-sm font-semibold text-slate-700">꿈 해몽</span>
        </div>
      </header>
        ...
        <div className="max-w-2xl mx-auto px-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3.5 rounded-2xl bg-white/10 border border-white/20 text-white placeholder-white/40 text-base focus:outline-none focus:bg-white/15 focus:border-violet-400 transition-all"
            />
            {search && (
              <button
      ...
      <div className="max-w-2xl mx-auto px-4">
        ...
        {!search && (
          <div className="pt-5 pb-2">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-3">많이 찾는 꿈</p>
            <div className="flex flex-wrap gap-2">
              {POPULAR_KEYWORDS.map(k => (
                <button
                  key={k}
                  onClick={() => { setSearch(k); setSelected(null); }}
                  className="px-3 py-1.5 rounded-full bg-white border border-slate-200 text-sm font-semibold text-slate-700 hover:border-violet-400 hover:text-violet-600 transition-all shadow-sm"
                >
                  {k}
                </button>
              ))}
        ...
        {filtered.length === 0 && (
          <div className="text-center py-24">
            ...
            <p className="text-slate-500 font-semibold">검색 결과가 없어요</p>
            <p className="text-slate-400 text-sm mt-1">다른 키워드로 검색해 보세요</p>
          </div>
        )}
            ...
            {filtered.map(dream => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-sm font-black text-slate-900 mb-1.5">{dream.keyword}</div>
                  <span className={`inline-block text-xs font-bold px-2 py-0.5 rounded-full mb-2 border ${li.bg} ${li.color} ${li.border}`}>
                    {li.label}
                  </span>
                  <p className="text-xs text-slate-500 leading-relaxed line-clamp-2">{dream.summary}</p>
                </button>
              );
            })}
          </div>
      ...
      {selected && info && (
        ...
        >
          ...
          {/* 시트 */}
          <div className="relative bg-white rounded-t-3xl w-full max-w-2xl max-h-[82vh] overflow-y-auto shadow-2xl">
            {/* 드래그 핸들 */}
            ...
            <div className="px-5 pb-10">
              {/* 헤더 */}
              <div className="flex items-start gap-4 py-5 border-b border-slate-100">
                <div className="text-5xl">{selected.emoji}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <h2 className="text-2xl font-black text-slate-900">{selected.keyword} 꿈</h2>
                    <span className={`text-sm font-bold px-2.5 py-1 rounded-full border ${info.bg} ${info.color} ${info.border}`}>
                      {info.label}
                    </span>
                  </div>
                  <p className="text-slate-500 text-sm font-semibold leading-snug">{selected.summary}</p>
                  <span className="text-xs text-slate-400 mt-1 inline-block">#{selected.category}</span>
                </div>
              </div>
              ...
              {selected.related && selected.related.length > 0 && (
                <div className="pt-4 border-t border-slate-100">
                  <p className="text-xs font-bold text-slate-400 mb-3">관련 꿈 더 보기</p>
                  <div className="flex flex-wrap gap-2">
                    {selected.related.map(rel => (
                      <button
                        key={rel}
                        onClick={() => { setSearch(rel); setSelected(null); }}
                        className="px-3 py-1.5 rounded-full bg-slate-100 text-sm font-semibold text-slate-600 hover:bg-violet-100 hover:text-violet-700 transition-colors"
                      >
                        {rel}
                      </button>
                    ))}
              ...
              <div className="flex gap-2 mt-6">
                ...
                <button
                  onClick={() => setSelected(null)}
                  className="flex-1 py-3.5 rounded-2xl bg-slate-100 font-bold text-slate-700 hover:bg-slate-200 transition-colors text-sm"
                >
                  닫기
                </button>
              </div>
