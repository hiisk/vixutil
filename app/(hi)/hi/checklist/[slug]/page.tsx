...
export function generateStaticParams() {
  return CHECKLISTS_INTL['hi'].map(c => ({ slug: c.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  return checklistIntlDetailMeta('hi', slug);
}

export default async function ChecklistPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const checklist = CHECKLISTS_INTL_MAP['hi'][slug];
  if (!checklist) notFound();
  return <ChecklistIntlDetail lang="hi" checklist={checklist} />;
}
