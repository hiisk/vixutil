...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!tool) return {};
  const text = textOf(tool, 'es');
  return {
    title: text.title,
    description: text.long,
    openGraph: openGraphFor('es'),
    alternates: {
      canonical: localeHref('es', `/geometry/${slug}`),
      languages: sectionAlternates('geometry', slug, GEO_LANGS),
    },
  };
}

export default async function GeoDetailEs({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const tool = geoTool(slug);
  if (!tool) notFound();
  return <FormulaPage tool={tool} lang="es" section={GEO_SECTION} Engine={GeoEngine} />;
}
