...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const heading = idiomHeading(i, 'es');
  return withCard({
    ...
    alternates: { canonical: localeHref('es', `/hanja/${slug}`), languages: hanjaAlternates(slug) },
  });
}

export default async function HanjaDetailEs({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const i = idiomBySlug(slug);
  if (!i) notFound();
  return <HanjaPage idiom={i} lang="es" />;
}
