...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return withCard({
    ...
    alternates: { canonical: localeHref('ja', `/hanja/${slug}`), languages: hanjaAlternates(slug) },
  });
}

export default async function HanjaDetailJa({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const i = idiomBySlug(slug);
  if (!i) notFound();
  return <HanjaPage idiom={i} lang="ja" />;
}
