...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!tool) return {};
  const text = textOf(tool, 'ja');
  return {
    title: text.title,
    description: text.long,
    openGraph: openGraphFor('ja'),
    alternates: {
      canonical: localeHref('ja', `/geometry/${slug}`),
      languages: sectionAlternates('geometry', slug, GEO_LANGS),
    },
  };
}

export default async function GeoDetailJa({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const tool = geoTool(slug);
  if (!tool) notFound();
  return <FormulaPage tool={tool} lang="ja" section={GEO_SECTION} Engine={GeoEngine} />;
}
