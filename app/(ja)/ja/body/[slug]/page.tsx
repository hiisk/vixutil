...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: localeHref('ja', `/body/${slug}`),
      languages: sectionAlternates('body', slug, BODY_LANGS),
    },
  };
}

export default async function BodyDetailJa({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const tool = bodyTool(slug);
  if (!tool) notFound();
  return <FormulaPage tool={tool} lang="ja" section={BODY_SECTION} Engine={BodyEngine} />;
}
