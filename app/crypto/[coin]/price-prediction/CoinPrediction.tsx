...
const VOL_CLR: Record<string, string> = {
  Low: 'text-emerald-600 dark:text-emerald-400', Medium: 'text-amber-600 dark:text-amber-400', High: 'text-orange-600 dark:text-orange-400', Extreme: 'text-rose-600 dark:text-rose-400',
};
    ...
    <>
      ...
      */}
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white/60 dark:bg-slate-900/60 px-4 py-3 mb-5 flex flex-wrap items-center gap-x-6 gap-y-2 text-[11px]">
        ...
        <span className="text-slate-500 dark:text-slate-400">Our stated 50% band actually contains <b className="text-emerald-600 dark:text-emerald-400">50.1%</b> of outcomes</span>
        <span className="text-slate-700 dark:text-slate-200">·</span>
        <span className="text-slate-500 dark:text-slate-400">Our 50% touch level is touched <b className="text-emerald-600 dark:text-emerald-400">50.0%</b> of the time</span>
        <span className="text-slate-700 dark:text-slate-200">·</span>
        <span className="text-slate-500 dark:text-slate-400">The moving-average + RSI + MACD method other sites use: <b className="text-rose-600 dark:text-rose-400">49.4%</b> directional accuracy</span>
      </div>
      ...
      {/* Section nav */}
      <nav className="sticky top-14 z-20 -mx-4 px-4 py-2 bg-slate-50/90 dark:bg-slate-950/90 backdrop-blur border-b border-slate-200 dark:border-slate-700 mb-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <label className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                ...
                <input
                  ...
                  className="w-28 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg px-2.5 py-1.5 text-sm text-slate-800 dark:text-slate-100 tabular-nums focus:outline-none focus:border-amber-500/60"
                />
              </label>
        ...
        {s.consensus && (
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4">
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-2">
              This drives the entry / TP / SL levels on the signal board. It deliberately does <b className="text-slate-500 dark:text-slate-400">not</b> feed the projections below —
      ...
      <Section id="prediction" title="Prediction" sub={`Two independent views: a conservative statistical model, and every comparable window ${coin.base} has actually lived through`}>
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 mb-4">
          ...
          <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-2 text-center leading-relaxed">
            ...
            every such move {coin.base} has actually made. It wanders up and down, but <b className="text-slate-500 dark:text-slate-400">do not read meaning into the wiggles</b>: we
                ...
                {series.map((d, ri) => {
                  ...
                  return (
                    <tr key={d.day} className="border-b border-slate-200/50 dark:border-slate-700/50 hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="px-3 py-2 text-right tabular-nums">
                        {dayChg != null ? (
                          <span className={`inline-flex items-center gap-1 font-bold ${dayChg >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                      ...
                      <td className="px-3 py-2 text-right tabular-nums text-amber-600 dark:text-amber-400 font-bold border-l border-slate-200/40 dark:border-slate-700/40">${formatPrice(seriesPeaks[ri] ?? d.forecast)}</td>
                    </tr>
                  );
                })}
              </tbody>
              ...
              <tbody>
                {m.projections.map(p => (
                  <tr key={p.key} className="border-b border-slate-200/50 dark:border-slate-700/50 hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-3 py-3 text-right tabular-nums border-l border-slate-200/40 dark:border-slate-700/40">
                      <span className="text-amber-600 dark:text-amber-400 font-bold">${formatPrice(p.peak)}</span>
                      <span className="block text-[10px] text-amber-500/60">+{p.peakPct.toFixed(1)}%</span>
                    </td>
                    <td className="px-3 py-3 text-right text-emerald-600 dark:text-emerald-400/80 tabular-nums border-l border-slate-200/40 dark:border-slate-700/40">{p.pUp10.toFixed(1)}%</td>
                    <td className="px-3 py-3 text-right text-rose-600 dark:text-rose-400/80 tabular-nums">{p.pDown10.toFixed(1)}%</td>
                    <td className="px-4 py-3 text-right text-[11px] text-slate-500 dark:text-slate-400 tabular-nums border-l border-slate-200/40 dark:border-slate-700/40">
                    ...
                    return (
                      <tr key={v} className={`border-b border-slate-200/50 dark:border-slate-700/50 hover:bg-slate-800/40 transition-colors ${isCustom ? 'bg-amber-500/[0.06]' : ''}`}>
                        ...
                        {BARRIER_CHECKPOINTS.map((d, i) => {
                          ...
                          return (
                            <td key={`e${d}`} className={`px-3 py-2.5 text-right tabular-nums font-bold ${clr} ${i === 0 ? 'border-l border-slate-200/40 dark:border-slate-700/40' : ''}`}>
                        ...
                        {BARRIER_CHECKPOINTS.map((d, i) => {
                          ...
                          return (
                            <td key={`c${d}`} className={`px-3 py-2.5 text-right tabular-nums ${clr} opacity-60 ${i === 0 ? 'border-l border-slate-200/40 dark:border-slate-700/40' : ''}`}>
                              {isFinite(p) ? `${p.toFixed(1)}%` : '-'}
                            </td>
                          );
                        })}
                ...
                <tbody>
                  {s.scenarios.map(r => (
                    <tr key={r.key} className="border-b border-slate-200/50 dark:border-slate-700/50 hover:bg-slate-800/40 transition-colors">
                      ...
                      {r.reliable ? (
                        <>
                          <td className="px-3 py-3 text-right text-rose-600 dark:text-rose-400/80 tabular-nums">${formatPrice(r.p25)}</td>
                          <td className="px-3 py-3 text-right text-slate-900 dark:text-white font-bold tabular-nums">${formatPrice(r.median)}</td>
                          <td className="px-3 py-3 text-right text-emerald-600 dark:text-emerald-400/80 tabular-nums">${formatPrice(r.p75)}</td>
                          <td className="px-3 py-3 text-right"><Pct value={r.medianPct} /></td>
                          <td className="px-3 py-3 text-right text-slate-500 dark:text-slate-400 tabular-nums">{r.pUp.toFixed(0)}%</td>
                          <td className="px-4 py-3 text-right text-[11px] tabular-nums border-l border-slate-200/40 dark:border-slate-700/40 text-slate-500 dark:text-slate-400">
                ...
                <tbody>
                  {months.map(r => (
                    <tr key={r.label} className="border-b border-slate-200/50 dark:border-slate-700/50 hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="px-3 py-2.5 text-right text-emerald-600 dark:text-emerald-400/80 tabular-nums">${formatPrice(r.high)}</td>
                      <td className="px-3 py-2.5 text-right text-amber-600 dark:text-amber-400 font-bold tabular-nums border-l border-slate-200/40 dark:border-slate-700/40">${formatPrice(r.peak)}</td>
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 mb-4">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
              ...
              <div className="text-right text-xs tabular-nums">
                <p className="text-emerald-600 dark:text-emerald-400 font-bold">{ta.s.bullish} bullish</p>
                <p className="text-rose-600 dark:text-rose-400 font-bold">{ta.s.bearish} bearish</p>
              ...
              .map(([title, rows]) => (
                <div key={title} className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-hidden">
                  ...
                  <table className="w-full text-sm">
                    <tbody>
                      {rows.map(r => (
                        <tr key={r.name} className="border-b border-slate-200/50 dark:border-slate-700/50 last:border-0">
                ...
                <table className="w-full text-sm">
                  <tbody>
                    {([['R3', ta.pv.r3], ['R2', ta.pv.r2], ['R1', ta.pv.r1]] as [string, number][]).map(([k, v]) => (
                      <tr key={k} className="border-b border-slate-200/50 dark:border-slate-700/50 last:border-0">
                        <td className="px-4 py-2 text-slate-500 dark:text-slate-400">Resistance {k}</td>
                        <td className="px-4 py-2 text-right text-emerald-600 dark:text-emerald-400 font-bold tabular-nums">${formatPrice(v)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <tbody>
                    {([['S1', ta.pv.s1], ['S2', ta.pv.s2], ['S3', ta.pv.s3]] as [string, number][]).map(([k, v]) => (
                      <tr key={k} className="border-b border-slate-200/50 dark:border-slate-700/50 last:border-0">
                        ...
                        <td className="px-4 py-2 text-right text-rose-600 dark:text-rose-400 font-bold tabular-nums">${formatPrice(v)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                {recent.map(k => {
                  ...
                  return (
                    <tr key={k.openTime} className="border-b border-slate-200/50 dark:border-slate-700/50 hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="px-3 py-2.5 text-right text-rose-600 dark:text-rose-400/70 tabular-nums">${formatPrice(k.low)}</td>
                      <td className="px-3 py-2.5 text-right text-slate-900 dark:text-white font-bold tabular-nums">${formatPrice(k.close)}</td>
                      <td className="px-3 py-2.5 text-right"><Pct value={chg} /></td>
                      <td className="px-4 py-2.5 text-right text-slate-500 dark:text-slate-400 tabular-nums">{formatVolume(k.quoteVolume)}</td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-900/50 p-4 mb-5 text-xs text-slate-500 dark:text-slate-400 leading-relaxed [&>p]:max-w-[95ch]">
        ...
        <p className="mb-2">
          We do <b className="text-slate-500 dark:text-slate-400">not</b> tilt the median in any direction, because we measured whether we could. Backtesting the technical
        ...
        <p className="mb-2">
          Direction is unpredictable, but <b className="text-slate-500 dark:text-slate-400">volatility is not</b> — it is close to the only thing in markets that genuinely
        ...
        <p className="mb-2">
          ...
          backtest showed our stated &quot;50%&quot; band actually contained <b className="text-slate-500 dark:text-slate-400">60.7%</b> of one-day outcomes. We therefore fit a
          ...
          over). After the fix the stated 50% and 80% bands contain <b className="text-slate-500 dark:text-slate-400">50.1%</b> and <b className="text-slate-500 dark:text-slate-400">79.0%</b> of
          outcomes at one day. The bands now mean what they say.
        </p>
        <p>
          We display the <b className="text-slate-500 dark:text-slate-400">50% interval (P25–P75)</b> by default and the 80% interval alongside it. We also tested whether mean
        ...
        <p className="mb-2">
          That is also why this page shows <b className="text-slate-500 dark:text-slate-400">two</b> things. The model forecast is parametric and conservative: one shrunken drift,
          so its path is necessarily smooth and one-directional. The <b className="text-slate-500 dark:text-slate-400">historical scenarios</b> table assumes no model at all — it
        ...
        <p>
          ...
          per-coin coefficients came out with <b className="text-slate-500 dark:text-slate-400">opposite signs</b> at every horizon (5, 20, 60 and 120 days) and neither half of
          ...
          <div className="flex flex-wrap gap-2">
            {corrs.map(({ base, r }) => {
              ...
              return meta ? (
                ...
                <div key={base} className="flex items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-950/60 px-3 py-2">{inner}</div>
              );
            })}
          </div>
        </div>
        ...
        <ul className="space-y-2 text-xs text-slate-500 dark:text-slate-400">
          {[
            ...
            <li key={claim} className="flex gap-2">
              <span className="text-rose-600 dark:text-rose-400/70 shrink-0" aria-hidden="true">✕</span>
              <span><b className="text-slate-700 dark:text-slate-200">{claim}.</b> <span className="text-slate-500 dark:text-slate-400">{why}</span></span>
            </li>
          ))}
        </ul>
      </div>
