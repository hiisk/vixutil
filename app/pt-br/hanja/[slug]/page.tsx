...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const heading = idiomHeading(i, 'pt-br');
  return {
    ...
    alternates: { canonical: localeHref('pt-br', `/hanja/${slug}`), languages: hanjaAlternates(slug) },
  };
}

export default async function HanjaDetailPtBr({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const i = idiomBySlug(slug);
  if (!i) notFound();
  return <HanjaPage idiom={i} lang="pt-br" />;
}
