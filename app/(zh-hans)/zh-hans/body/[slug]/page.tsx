...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: localeHref('zh-hans', `/body/${slug}`),
      languages: sectionAlternates('body', slug, BODY_LANGS),
    },
  };
}

export default async function BodyDetailZhHans({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const tool = bodyTool(slug);
  if (!tool) notFound();
  return <FormulaPage tool={tool} lang="zh-hans" section={BODY_SECTION} Engine={BodyEngine} />;
}
