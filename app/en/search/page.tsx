...
export default function EnSearchPage() {
  const items = searchIndexIntl('en');
  return (
    <div className="relative min-h-screen bg-white dark:bg-slate-900 flex flex-col">
      ...
      <header className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-3">
          <Link href="/en" className="font-black text-indigo-600 text-lg shrink-0">vix.</Link>
      ...
      <div className="flex-1 max-w-2xl mx-auto px-4 py-8 w-full">
        ...
        <GlobalSearchIntl items={items} lang="en" />
      </div>

      <footer className="border-t border-slate-100 dark:border-slate-800 py-8 text-center">
        <Link href="/en" className="text-sm font-black text-indigo-600">vixutil</Link>
      </footer>
    </div>
  );
}
