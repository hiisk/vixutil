...
export const metadata: Metadata = {
  title: 'Image Tools — Compress, Resize, Crop, Blur',
...
const CATEGORY_ORDER = ['Size', 'Edit', 'Analyse'];

export default function EnImageHub() {
  const tools = imageToolsIntl('en');
  const ui = IMAGE_SHELL_UI['en'];
    ...
    <div className="relative min-h-screen bg-white dark:bg-slate-900">
      ...
      <header className="sticky top-0 z-10 bg-white/95 dark:bg-slate-900/95 backdrop-blur border-b border-slate-100 dark:border-slate-800">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center gap-4">
          <Link href="/en" className="font-black text-violet-600 text-lg shrink-0">vixutil</Link>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        ...
        <h1 className="text-3xl font-black text-slate-900 dark:text-slate-100 mb-2">Image Tools</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mb-8">Compress, crop and edit photos — nothing leaves your browser.</p>
        ...
        {grouped.map(g => (
          <section key={g.category} className="mb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {g.tools.map(t => (
                <Link key={t.slug} href={`/en/image/${t.slug}`}
      ...
      <footer className="border-t border-slate-100 dark:border-slate-800 py-8 text-center">
        ...
        <p className="text-xs text-slate-400 mt-1">Free image tools</p>
      </footer>
    </div>
  );
}
