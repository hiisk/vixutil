...
export const metadata: Metadata = {
  title: 'Sound Tools — Metronome, Tuner, White Noise',
...
const CATEGORY_ORDER = ['Practice', 'Focus and sleep', 'Measure', 'Signals'];

export default function EnSoundHub() {
  const tools = soundToolsIntl('en');
  const ui = SOUND_SHELL_UI['en'];
  ...
  return (
    <div className="relative min-h-screen bg-white dark:bg-slate-900">
      ...
      <header className="sticky top-0 z-10 bg-white/95 dark:bg-slate-900/95 backdrop-blur border-b border-slate-100 dark:border-slate-800">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center gap-4">
          ...
          <span className="ml-auto flex items-center gap-2 text-xs font-bold text-slate-400">
            ...
            <Link href="/zh/sound" className="hover:text-indigo-600" hrefLang="zh">中文</Link>
          </span>
        </div>
      </header>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        ...
        <h1 className="text-3xl font-black text-slate-900 dark:text-slate-100 mb-2">Sound Tools</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mb-8">Metronome, tuner and noise — every sound generated in your browser.</p>
        ...
        {grouped.map(g => (
          <section key={g.category} className="mb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {g.tools.map(t => (
                <Link key={t.slug} href={`/en/sound/${t.slug}`}
      ...
      <footer className="border-t border-slate-100 dark:border-slate-800 py-8 text-center">
        ...
        <p className="text-xs text-slate-400 mt-1">Free sound tools</p>
      </footer>
    </div>
  );
}
