...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: localeHref('fr', `/rate/${slug}`),
      languages: sectionAlternates('rate', slug, RATE_LANGS),
    },
  };
}

export default async function RateDetailFr({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const tool = rateTool(slug);
  if (!tool) notFound();
  return <FormulaPage tool={tool} lang="fr" section={RATE_SECTION} Engine={RateEngine} />;
}
