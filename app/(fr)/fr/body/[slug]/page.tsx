...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return withCard({
    ...
    alternates: {
      canonical: localeHref('fr', `/body/${slug}`),
      languages: sectionAlternates('body', slug, BODY_LANGS),
    },
  });
}

export default async function BodyDetailFr({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const tool = bodyTool(slug);
  if (!tool) notFound();
  return <FormulaPage tool={tool} lang="fr" section={BODY_SECTION} Engine={BodyEngine} />;
}
