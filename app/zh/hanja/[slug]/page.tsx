...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const heading = idiomHeading(i, 'zh');
  return {
    title: heading === i.hanja ? `${i.hanja} — ${ui.section}` : `${i.hanja} ${heading} — ${ui.section}`,
    description: `${t.meaning} ${t.origin}`,
    alternates: { canonical: '/zh/hanja/' + slug, languages: hanjaAlternates(slug) },
  };
}

export default async function HanjaDetailZH({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const i = idiomBySlug(slug);
  if (!i) notFound();
  return <HanjaPage idiom={i} lang="zh" />;
}
