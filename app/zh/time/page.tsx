...
export const metadata: Metadata = {
  title: '时间工具 — 计时器、秒表、世界时钟',
  description: '免费时间工具：计时器、秒表、番茄钟、闹钟、世界时钟、时区换算、工作日与日期计算。在浏览器内运行，无需安装。',
  alternates: {
    canonical: '/zh/time',
...
const CATEGORY_ORDER = ['计时', '世界时间', '日期计算'];

export default function ZhTimeHub() {
  const tools = timeToolsIntl('zh');
  const ui = TIME_SHELL_UI['zh'];
    ...
    <div className="relative min-h-screen bg-white dark:bg-slate-900">
      ...
      <header className="sticky top-0 z-10 bg-white/95 dark:bg-slate-900/95 backdrop-blur border-b border-slate-100 dark:border-slate-800">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center gap-4">
          <Link href="/zh" className="font-black text-sky-600 text-lg shrink-0">vixutil</Link>
          ...
          <span className="ml-auto flex items-center gap-2 text-xs font-bold text-slate-400">
            ...
            <Link href="/en/time" className="hover:text-sky-600" hrefLang="en">EN</Link>
          </span>
        </div>
      </header>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        ...
        <h1 className="text-3xl font-black text-slate-900 dark:text-slate-100 mb-2">时间工具</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mb-8">计时、看时间、算日期 —— 全部在浏览器内运行。</p>
        ...
        {grouped.map(g => (
          <section key={g.category} className="mb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {g.tools.map(t => (
                <Link key={t.slug} href={`/zh/time/${t.slug}`}
      ...
      <footer className="border-t border-slate-100 dark:border-slate-800 py-8 text-center">
        ...
        <p className="text-xs text-slate-400 mt-1">免费时间工具</p>
      </footer>
    </div>
  );
}
