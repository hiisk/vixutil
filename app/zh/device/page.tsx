...
const CATEGORY_ORDER = ['输入设备', '音频', '视频', '屏幕', '信息'];

export default function ZhDeviceHub() {
  const tools = deviceToolsIntl('zh');
  const ui = DEVICE_SHELL_UI['zh'];
  ...
  return (
    <div className="relative min-h-screen bg-white dark:bg-slate-900">
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        ...
        <h1 className="text-3xl font-black text-slate-900 dark:text-slate-100 mb-2">设备检测</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mb-8">在浏览器里检查硬件 —— 无需安装任何东西。</p>
        ...
        {grouped.map(g => (
          <section key={g.category} className="mb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {g.tools.map(t => (
                <Link key={t.slug} href={`/zh/device/${t.slug}`}
      ...
      <footer className="border-t border-slate-100 dark:border-slate-800 py-8 text-center">
        ...
        <p className="text-xs text-slate-400 mt-1">免费设备检测</p>
      </footer>
    </div>
  );
}
