...
const CATEGORY_ORDER = ['整理与编辑', '符号与输入', '统计与写作'];

export default function ZhTextHub() {
  const tools = textToolsIntl('zh');
  const ui = TEXT_SHELL_UI['zh'];
  ...
  return (
    <div className="relative min-h-screen bg-white dark:bg-slate-900">
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        ...
        <h1 className="text-3xl font-black text-slate-900 dark:text-slate-100 mb-2">文本工具</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mb-8">清理、转换与统计 —— 全部在浏览器内完成。</p>
        ...
        {grouped.map(g => (
          <section key={g.category} className="mb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {g.tools.map(t => (
                <Link key={t.slug} href={`/zh/text/${t.slug}`}
      ...
      <footer className="border-t border-slate-100 dark:border-slate-800 py-8 text-center">
        ...
        <p className="text-xs text-slate-400 mt-1">免费文本工具</p>
      </footer>
    </div>
  );
}
