    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="bg-white border-b border-slate-100 sticky top-0 z-10">
        <div className="max-w-xl mx-auto px-4 h-14 flex items-center gap-3">
          <Link href="/snap" className="flex items-center gap-1.5 text-sm text-slate-400 hover:text-pink-600 transition-colors font-medium">
          ...
          <span className="text-sm font-semibold text-slate-700">표정 감정 분석</span>
        </div>
      </header>
      ...
      <div className="max-w-xl mx-auto px-4 py-8">
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-black text-slate-900 mb-1.5">표정 감정 분석</h1>
          <p className="text-slate-500 text-sm">AI 감정 인식 모델로 사진 속 표정을 7가지 감정으로 분석해요</p>
        </div>
        ...
        {modelState === 'loading' && (
          <div className="w-full border-2 border-dashed border-slate-200 rounded-2xl py-16 flex flex-col items-center gap-3 bg-white">
            ...
            <span className="text-sm font-bold text-slate-500">감정 인식 모델을 불러오는 중...</span>
          </div>
        )}
        ...
        {modelState === 'ready' && !preview && (
          <button
            ...
            className="w-full border-2 border-dashed border-slate-300 rounded-2xl py-16 flex flex-col items-center gap-3 bg-white hover:border-pink-400 hover:bg-pink-50/50 transition-colors"
          >
            <span className="text-4xl">📷</span>
            <span className="text-sm font-bold text-slate-600">사진을 선택해주세요</span>
            <span className="text-xs text-slate-400">표정이 잘 보이는 정면 사진일수록 정확해요</span>
          </button>
        )}
        ...
        {preview && (
          <div className="mb-6">
            ...
            {!analyzing && (
              <button onClick={() => fileInputRef.current?.click()} className="mt-3 mx-auto block text-xs font-semibold text-slate-400 hover:text-pink-600 transition-colors">
                다른 사진으로 다시 보기
              </button>
            )}
          </div>
        ...
        {result && !analyzing && (
          <div id="expr-result" className="space-y-4">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">😀 감정 분포</p>
              <div className="flex flex-col gap-2.5">
                {result.scores.map(s => (
                  <div key={s.key}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-semibold text-slate-600">{s.emoji} {s.label}</span>
                      <span className="text-xs font-bold text-slate-500">{s.percent}%</span>
                    </div>
            ...
            <button onClick={handleReset} className="w-full py-3.5 rounded-2xl font-bold text-sm bg-white border-2 border-slate-200 text-slate-600 hover:border-pink-300 hover:text-pink-600 transition-colors">
              🔄 다른 사진으로 다시 해보기
            </button>
