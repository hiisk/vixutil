...
export default function BloodMatchPage() {
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <div className="hero-band max-w-xl mx-auto px-4 py-6">
        ...
        <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 mb-4 flex flex-col gap-5">
          ...
          <button
            onClick={calculate}
            disabled={!me || !partner}
            className="w-full rounded-xl bg-sec disabled:from-slate-200 disabled:to-slate-200 dark:disabled:from-slate-800 dark:disabled:to-slate-800 disabled:text-slate-500 dark:text-slate-400 text-sm font-bold py-3.5 transition-all active:scale-[0.99] shadow-sm shadow-rose-200 disabled:shadow-none"
          >
      ...
      <RelatedContent items={FORTUNE_RELATED} currentSlug="blood-match" basePath="/fortune" accent="violet" bg="" />
      <SiteFooter />
    </div>
  );
}
