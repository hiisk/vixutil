...
export default function GasCostPage() {
   ...
   * 예전에 버튼을 안 누른 상태와 같다.
   */
  const result: null | { fuelNeeded: number; cost: number } = ((): null | { fuelNeeded: number; cost: number } => {
    const d = Number(distance);
    const e = Number(efficiency);
    const p = Number(fuelPrice);
    if (d <= 0 || e <= 0 || p <= 0) return null;
    const dist = roundTrip ? d * 2 : d;
    const fuelNeeded = dist / e;
    return ({ fuelNeeded, cost: fuelNeeded * p });
  ...
  return (
    ...
    >
      <div className="flex justify-end mb-4">
        <LangPicker current="ko" route="/calculator/gas-cost" available={ALL_LOCALES10} />
      </div>
