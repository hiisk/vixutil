...
export default function RefinancePage() {
  ...
  const [currentMonths, setCurrentMonths] = useState('120');
   ...
   * 예전에 버튼을 안 누른 상태와 같다.
   */
  const result: RefinanceResult | null = ((): RefinanceResult | null => {
    if (balance <= 0 || !currentRate || !currentMonths || !newRate || !newMonths) return null;
    return (
      compareRefinance({
        balance,
  ...
  return (
    <CalcShell
      ...
      intro={
        <>
          <h2>금리만 보면 손해를 봅니다</h2>
          <p>
            이자가 싸지는 건 맞는데, 갈아타는 데 드는 돈이 따로 있습니다.{' '}
            <strong>중도상환수수료</strong>와 신규 대출의 <strong>인지세·근저당 설정비 같은 부대비용</strong>을
            합친 초기 비용이 먼저 나가기 때문입니다. 금리 차이가 작으면 이 비용을 회수하기 전에 대출이 끝나
            오히려 손해가 됩니다.
          </p>
          <h2>손익분기점이 진짜 기준입니다</h2>
          <p>
            이 계산기는 매달 아끼는 이자로 초기 비용을 <strong>몇 달 만에 회수하는지</strong>를 계산합니다.
            손익분기점이 남은 기간보다 짧으면 이득이고, 길면 갈아타지 않는 편이 낫습니다.
            중도상환수수료는 보통 대출 실행 후 3년이 지나면 없어지므로, 3년이 가까웠다면 조금 기다리는 것만으로
            조건이 달라집니다.
          </p>
          <h2>계산 조건</h2>
          <p>
            <strong>원리금균등상환</strong> 기준으로 계산합니다. 금융사마다 이자 일할계산 방식이 달라 실제
            금액은 소폭 차이가 날 수 있습니다. 변동금리 대출이라면 지금 금리로 남은 기간을 계산한 값이므로
            앞으로 금리가 바뀌면 결과도 달라집니다.
          </p>
        </>
      }
    >
