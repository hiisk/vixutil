                    ...
                    {QUICK_RATES.map(r => (
                      <button key={r} type="button" onClick={() => setDiscRate(String(r))}
                        className={`px-3.5 py-2 text-sm font-semibold rounded-xl border transition-colors ${
                          discRate === String(r) ? 'bg-blue-600 border-blue-600 text-white' : 'border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700'
                        }`}>
                        {r}%
                      </button>
                    ))}
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {QUICK_RATES.map(r => (
                      <button key={r} type="button" onClick={() => setDiscRate3(String(r))}
                        className={`px-3.5 py-2 text-sm font-semibold rounded-xl border transition-colors ${
                          discRate3 === String(r) ? 'bg-blue-600 border-blue-600 text-white' : 'border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700'
                        }`}>
                        {r}%
                      </button>
                    ))}
