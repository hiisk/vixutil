    ...
    <CalcShell
      ...
      intro={
        <>
          <h2>계산 방식</h2>
          <p>
            퇴직금은 <strong>1일 평균임금 × 30일 × (재직일수 ÷ 365)</strong>입니다. 1년 일하면 대략 한 달치
            급여를 받는 구조입니다. 1일 평균임금은 <strong>퇴직 전 3개월 임금 총액을 그 3개월의 총 일수로
            나눈</strong> 값이라, 월수가 아니라 실제 달력 일수로 나눕니다.
          </p>
          <h2>상여금과 연차수당도 들어갑니다</h2>
          <p>
            3개월 급여만 넣고 계산하면 퇴직금이 실제보다 적게 나옵니다. <strong>연간 상여금</strong>과{' '}
            <strong>연간 연차수당</strong>도 3개월치에 해당하는 만큼(× 3개월 일수 ÷ 365) 평균임금에 포함되기
            때문입니다. 상여금이 큰 회사일수록 이 차이가 커집니다.
          </p>
          <h2>통상임금이 더 높으면 그쪽을 씁니다</h2>
          <p>
            퇴직 직전 3개월에 무급휴직이나 결근이 있었다면 평균임금이 평소보다 낮게 나옵니다. 법은 이런 경우
            근로자가 손해 보지 않도록 <strong>평균임금이 통상임금보다 낮으면 통상임금을 기준</strong>으로
            하도록 정해뒀습니다. 이 계산기도 둘 중 큰 값을 씁니다.
          </p>
          <h2>받을 수 있는 조건</h2>
          <p>
            <strong>계속근로 1년 이상</strong>이고 <strong>주 15시간 이상</strong> 일해야 대상입니다.
            1년에서 며칠 모자라면 못 받으므로 퇴사일을 정할 때 확인할 값어치가 있습니다. 퇴직연금(DC형)에
            가입돼 있다면 회사가 매년 납입한 금액과 운용 수익이 퇴직급여가 되어 이 계산과 다릅니다.
          </p>
        </>
      }
    >
      <div className="flex flex-col gap-4">
        ...
        {error && (
          <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/50 rounded-xl px-4 py-3 text-sm text-red-700 dark:text-red-300">{error}</div>
        )}
