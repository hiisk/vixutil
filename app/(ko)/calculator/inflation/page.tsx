...
export default function InflationPage() {
  ...
  return (
    <CalcShell
      ...
      wide
      title="물가상승률 계산기"
      description="인플레이션에 따른 미래 필요 금액과 실질 구매력 변화를 계산합니다"
      intro={
        <>
          <h2>돈의 가치는 가만히 있어도 줄어듭니다</h2>
          <p>
            물가가 매년 3%씩 오르면, 지금 100만원으로 사던 것을 10년 뒤에는 약 134만원을 줘야 삽니다.
            반대로 말하면 <strong>10년 뒤의 100만원은 지금의 74만원어치</strong>입니다. 현금을 그냥
            들고 있으면 숫자는 그대로여도 살 수 있는 양은 계속 줄어듭니다.
          </p>
          <h2>두 방향으로 씁니다</h2>
          <p>
            <strong>미래 가치</strong>는 지금 금액이 나중에 얼마가 되어야 같은 값어치인지를 구합니다.
            은퇴자금이나 학자금 목표를 잡을 때 씁니다. <strong>현재 가치 역산</strong>은 반대로 미래
            금액이 지금 돈으로 얼마인지를 봅니다. &ldquo;30년 뒤 10억&rdquo;이 지금 감각으로 얼마인지
            확인할 때 유용합니다.
          </p>
          <h2>수익률은 물가를 넘겨야 의미가 있습니다</h2>
          <p>
            물가가 3% 오르는데 예금 이자가 3%면 <strong>실질 수익은 0</strong>입니다. 세금까지 떼면
            오히려 마이너스입니다. 투자 수익률을 볼 때 명목이 아니라{' '}
            <strong>물가를 뺀 실질 수익률</strong>로 따져야 하는 이유입니다.
          </p>
          <h2>일정한 상승률을 가정합니다</h2>
          <p>
            실제 물가상승률은 해마다 다르고 품목별로도 크게 갈립니다. 여기서 나온 값은 입력한 비율이
            계속 유지된다고 봤을 때의 <strong>추정</strong>이므로, 몇 가지 비율로 돌려보며 범위를
            잡는 편이 낫습니다.
          </p>
        </>
      }
                    ...
                    <table className="calc-table">
                      ...
                      <tbody>
                        {(showAllF ? futureRows! : futureRows!.slice(0, 20)).map(r => (
                          <tr key={r.year}>
                            <td>{r.year}년 후</td>
                            <td className="font-bold text-slate-900 dark:text-slate-100">{w(r.value)}원</td>
