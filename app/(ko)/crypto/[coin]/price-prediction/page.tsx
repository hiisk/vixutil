...
export default async function CoinPredictionPage({ params }: { params: Promise<{ coin: string }> }) {
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100">
      ...
      <header className="border-b border-slate-200 dark:border-slate-700 sticky top-0 z-30 bg-slate-50/90 dark:bg-slate-950/90 backdrop-blur">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center gap-3">
          ...
          <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">{coin.name}</span>
        </div>
      </header>
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        */}
        <section className="mt-10 rounded-lg border border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-900/50 p-5 note-sm [&>p]:max-w-[72ch]">
          <h2 className="text-base font-bold text-slate-900 dark:text-white mb-3">About the {coin.name} ({coin.base}) price prediction</h2>
          <p className="mb-3">
            ...
            weekly and monthly views. It shows two complementary things: a conservative statistical forecast, and a <b className="text-slate-700 dark:text-slate-200">historical
          ...
          <p className="mb-3">
            The forecast splits {coin.base}&apos;s trend into two parts: a <b className="text-slate-700 dark:text-slate-200">market component</b> (its beta to Bitcoin) and a
            <b className="text-slate-700 dark:text-slate-200"> coin-specific component</b> (alpha), each shrunk toward zero as a Bayesian posterior mean. The market drift is
            ...
            ahead across 24 coins, this setting is about <b className="text-slate-700 dark:text-slate-200">2.6% worse</b> in RMSE than simply assuming the price does not change, and
            ...
            We do <b className="text-slate-700 dark:text-slate-200">not</b> tilt the forecast with technical indicators. Backtested across 46 coins with non-overlapping forward
            ...
            <b className="text-slate-700 dark:text-slate-200">49.8%</b> — a coin flip. Momentum and reversal were no better: their signs flipped between pooled and per-coin fits.
          </p>
          <p>
            Two consequences are worth stating plainly. The forecast <b className="text-slate-700 dark:text-slate-200">extrapolates a trend that is not statistically significant</b>,
            ...
            <b className="text-slate-700 dark:text-slate-200"> smooth and monotone</b>: we tested a day-of-week effect to see whether a zig-zagging daily forecast could be justified, and on the market series no weekday
            reaches statistical significance. Direction is not forecastable, but <b className="text-slate-700 dark:text-slate-200">volatility is</b>, so each horizon uses its own
        ...
        <section className="mt-5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-900/50 p-5 [&_dd]:max-w-[72ch]">
          <h2 className="text-base font-bold text-slate-900 dark:text-white mb-3">{coin.name} price prediction FAQ</h2>
          <dl className="space-y-4 text-sm">
            {faqs(coin.name, coin.base).map(qa => (
              <div key={qa.q}>
                <dt className="font-bold text-slate-700 dark:text-slate-200 mb-1">{qa.q}</dt>
