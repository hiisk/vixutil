...
function ShareBtn() {
  ...
  const handleShare = useCallback(async () => {
    ...
    if (await shareOne('사진 한 장으로 보는 손글씨 심리 테스트 — vixutil.com')) {
      setState('copied');
      ...
      <div className="hero-band max-w-xl mx-auto px-4 py-8">
        ...
        {result && !analyzing && (
          <div id="hw-result" className="space-y-4">
            <div className="bg-gradient-to-br from-slate-700 to-indigo-800 rounded-lg p-6 text-white text-center">
            ...
            <button
              onClick={handleReset}
              className="w-full py-3.5 rounded-lg font-bold text-sm bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-indigo-300 hover:text-indigo-600 transition-colors"
            >
              🔄 다른 사진으로 다시 해보기
            </button>
