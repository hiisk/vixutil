...
function ShareBtn() {
  ...
  const handleShare = useCallback(async () => {
    ...
    if (await shareOne('사진 한 장으로 보는 얼굴 대칭 지수 — vixutil.com')) {
      setState('copied');
    ...
    <div className="relative min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <header className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 sticky top-0 z-10">
        <div className="max-w-xl mx-auto px-4 h-14 flex items-center gap-3">
          <Link href="/snap" className="page-back hover:text-indigo-600">
          ...
          <span className="ml-auto shrink-0">
            <LangPicker current="ko" route="/snap/face-symmetry" available={ALL_LOCALES10} />
          </span>
        </div>
      </header>
      ...
      <div className="hero-band max-w-xl mx-auto px-4 py-8">
        ...
        {faceError && !analyzing && (
          <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 rounded-lg p-5 mb-6 text-center">
            ...
            <button onClick={() => fileInputRef.current?.click()} className="text-sm font-bold text-amber-950 bg-amber-500 hover:bg-amber-600 rounded-xl px-4 py-2.5 transition-colors">
              다른 사진 선택하기
            </button>
          </div>
        )}
        ...
        {result && !analyzing && (
          <div id="symmetry-result" className="space-y-4">
            ...
            <button onClick={handleReset} className="w-full py-3.5 rounded-lg font-bold text-sm bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-700 hover:text-indigo-600 transition-colors">다른 사진으로 다시 해보기
            </button>
