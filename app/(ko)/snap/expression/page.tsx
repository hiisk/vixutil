...
function ShareBtn() {
  ...
  const handleShare = useCallback(async () => {
    ...
    if (await shareOne('사진 한 장으로 보는 표정 감정 분석 — vixutil.com')) {
      setState('copied');
    ...
    <div className="relative min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <header className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 sticky top-0 z-10">
        <div className="max-w-xl mx-auto px-4 h-14 flex items-center gap-3">
          ...
          <span className="ml-auto shrink-0">
            <LangPicker current="ko" route="/snap/expression" available={ALL_LOCALES10} />
          </span>
        </div>
      </header>
      ...
      <div className="hero-band max-w-xl mx-auto px-4 py-8">
        <div className="mb-6">
          ...
          <h1 className="page-h1">표정 감정 분석</h1>
        ...
        {modelState === 'ready' && !preview && (
          <button
            ...
            className="w-full border-2 border-dashed border-slate-300 rounded-lg py-16 flex flex-col items-center gap-3 bg-white dark:bg-slate-900 hover:border-pink-400 hover:bg-pink-50/50 dark:hover:bg-pink-950/40 transition-colors"
          >
        ...
        {result && !analyzing && (
          <div id="expr-result" className="space-y-4">
            ...
            <button onClick={handleReset} className="w-full py-3.5 rounded-lg font-bold text-sm bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-pink-300 hover:text-pink-600 transition-colors">
              🔄 다른 사진으로 다시 해보기
            </button>
