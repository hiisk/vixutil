...
export function generateStaticParams() {
  return GENERATORS_INTL['es'].map(g => ({ slug: g.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  return generatorIntlDetailMeta('es', slug);
}

export default async function GeneratorPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const gen = GENERATORS_INTL_MAP['es'][slug];
  if (!gen) notFound();
  return <GeneratorIntlDetail lang="es" gen={gen} />;
}
