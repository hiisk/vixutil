 ...
 *
 *   ISR 쓰기      48만~69만 단위 ← 무료 월 20만의 **240~343%**. 여기가 벽이다
 *   Origin 전송   3.3GB          (한 장 gzip 16.3KB) — 10GB의 33%
 *   활성 CPU      1.3~2.7 CPU-hr (한 장 23.5ms 실측 × Vercel 보정) — 4시간의 33~67%
 *   함수 호출     203,039        — 100만의 20%
 *
 * 나머지는 다 여유가 있고 **쓰기 하나가 한 바퀴에 한도를 두세 배 넘긴다**(쓰기는
 * 장수가 아니라 8KB 단위로 세고, 한 장이 2.36~3.38단위다 — 셈은 lib/prerender.ts).
 * 그러니 그 한 바퀴를 무료로 내려간 뒤에 맞으면 그 자리에서 30일 정지다.
 *
 * **이 스크립트만으로는 무료에 못 들어간다.** 첫 바퀴를 Pro에서 태워 캐시를
 * 채워 두는 것까지가 여기 몫이고, 그 뒤에도 배포할 때마다 캐시가 새로 생겨
 * 같은 값이 다시 든다. 페이지를 줄일지 ISR을 버릴지는 lib/prerender.ts의
 * 「남는 선택」 셋에 적었다.
 *
 * ── 왜 HEAD인가 ───────────────────────────────────────────────
