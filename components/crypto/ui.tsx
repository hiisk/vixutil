...
export function CoinLogo({ base, size = 24 }: { base: string; size?: number }) {
  ...
  if (failed) {
    return (
      <span style={style} className="shrink-0 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[9px] font-bold grid place-items-center">
        {base.slice(0, 2)}
      </span>
    );
  }
