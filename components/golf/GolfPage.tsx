    ...
    <div className="page-wrap">
      ...
      <header className="page-head">
        <div className="page-bar">
          ...
          <span className="row-name tabular-nums">{c.score} · {c.slope}</span>
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        ...
        <section className="mb-8">
          ...
          <p className="mt-2 note-xs">{ui.slopeNote}</p>
        </section>

        <section className="mb-8">
          <h2 className="sec-h2-tight">{ui.formulaTitle}</h2>
          <p className="note-xs">{ui.formulaNote}</p>
        </section>

        <section className="mb-8">
          <h2 className="sec-h2-tight">{ui.indexTitle}</h2>
          <p className="note-xs">{ui.indexNote}</p>
        </section>
        ...
        <nav className="foot-nav" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link prefetch={false} key={l.lang} href={`${l.prefix}/golf/${slug}`} hrefLang={l.hreflang} className="dim-link">
              {l.label}
            </Link>
          ))}
        </nav>
