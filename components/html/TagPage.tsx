...
export default function TagPage({ slug, lang }: { slug: string; lang: Lang }) {
  ...
  */
  ...
  const related = relatedTags(slug);

  const rows: { label: string; value: string }[] = [
    { label: ui.writeLabel, value: f.example },
    { label: ui.closeLabel, value: f.close || '—' },
    { label: ui.voidLabel, value: f.isVoid ? ui.voidYes : ui.voidNo },
    { label: ui.attrsLabel, value: f.attrs.length ? f.attrs.join(', ') : ui.attrsNone },
    ...
    <div className="page-wrap">
      <JsonLd
        data={breadcrumbJsonLd([
          ...
          { name: ui.section, path: `${prefix}/html` },
          { name: `<${t.name}>`, path },
        ])}
      />
      <JsonLd data={webAppJsonLd(`<${t.name}>`, ui.metaDesc(t.name, desc), path)} />
      ...
      <header className="page-head">
        <div className="page-bar">
          ...
          <Link prefetch={false} href={`${prefix}/html`} className="text-sm text-slate-400 dark:text-slate-500 hover:text-slate-700 transition-colors font-medium truncate">
            {ui.section}
          </Link>
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        <div className="text-center mb-6">
          <div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl mb-3 shadow-lg bg-gradient-to-br from-orange-500 to-amber-500">
            <ToolIcon emoji={TAG_ICON} accent="rgba(255,255,255,0.55)" className="w-6 h-6 text-white" />
          </div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-slate-100 mb-1 font-mono">&lt;{t.name}&gt;</h1>
        ...
        {f.deprecated && (
          <p className="text-xs text-amber-800 dark:text-amber-200 leading-relaxed rounded-2xl border border-amber-200 dark:border-amber-900/60 bg-amber-50 dark:bg-amber-950/30 px-4 py-3 mb-6">
            {ui.deprecatedWarn}
          </p>
        )}
        ...
        <Faq items={ui.tagFaq(f, desc, kind)} lang={base} title={ui.faqTitle} />
        ...
        <p className="mt-6 text-center">
          <a
            ...
            target="_blank"
            className="text-xs font-bold text-slate-400 dark:text-slate-500 hover:text-orange-600 transition-colors"
          >
            {ui.docLabel} ↗
          </a>
        </p>
        ...
        <nav className="foot-nav" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link prefetch={false} key={l.lang} href={`${l.prefix}/html/${slug}`} hrefLang={l.hreflang} className="dim-link">
              {l.label}
            </Link>
          ))}
        </nav>
