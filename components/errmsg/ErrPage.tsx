    ...
    <div className="page-wrap">
      ...
      <header className="page-head">
        <div className="page-bar">
          ...
          <Link prefetch={false} href={`${prefix}/error`} className="text-sm text-slate-400 dark:text-slate-500 hover:text-slate-700 transition-colors font-medium truncate">
            {ui.section}
          </Link>
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
            ...
            {f.related.map(s => {
              ...
              return (
                <Link prefetch={false}
                  key={s}
                  href={`${prefix}/error/${s}`}
                  className="rounded-xl border chip-off px-4 py-3 hover:border-rose-300 hover:shadow-sm transition-all"
                >
        ...
        <nav className="foot-nav" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link prefetch={false} key={l.lang} href={`${l.prefix}/error/${slug}`} hrefLang={l.hreflang} className="dim-link">
              {l.label}
            </Link>
          ))}
        </nav>
