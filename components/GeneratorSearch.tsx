  ...
  return (
    <>
      ...
      <div className="relative mb-5">
        <svg className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 dark:text-slate-400 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        />
        {query && (
          <button onClick={() => setQuery('')} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-500 dark:text-slate-400 hover:text-slate-600">
      ...
      {grouped ? (
            ...
            return (
              <section key={group.name}>
                <div className="flex items-center gap-2 mb-4">
                  <ToolIcon emoji={meta.icon} className="w-5 h-5 text-slate-500 dark:text-slate-400" />
                  <h2 className="text-sm font-bold text-slate-800 dark:text-slate-100">{group.name}</h2>
                  <span className="text-xs font-bold text-emerald-600 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/30 px-2.5 py-0.5 rounded-full">{group.items.length}</span>
                </div>
        ...
        <div className="py-20 text-center">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">&apos;{trimmed}&apos;에 해당하는 생성기가 없어요.</p>
