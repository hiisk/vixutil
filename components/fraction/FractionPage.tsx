    ...
    <div className="page-wrap">
      ...
      <header className="page-head">
        <div className="page-bar">
          ...
          <span className="row-name">{f.n}/{f.d}</span>
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        ...
        <section className="mb-8">
          <h2 className="sec-h2">{ui.sameDenomTitle}</h2>
          <div className="flex flex-wrap gap-1.5">
            {sameDenominator(fr).map(o => (
              <Link prefetch={false}
                key={slugOf(o)}
                href={`${hub}/${slugOf(o)}`}
                className="rounded-lg border chip-off px-2.5 py-1 text-xs font-bold text-slate-600 dark:text-slate-300 tabular-nums hover:border-lime-400 hover:text-lime-600 dark:hover:text-lime-400 transition-colors"
              >
                {o.n}/{o.d}
              </Link>
            ))}
        ...
        <nav className="foot-nav" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link prefetch={false} key={l.lang} href={`${l.prefix}/fraction/${slug}`} hrefLang={l.hreflang} className="dim-link">
              {l.label}
            </Link>
          ))}
        </nav>
