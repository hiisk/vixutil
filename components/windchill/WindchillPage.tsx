      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        ...
        {f.frostbite !== null && (
          <section className="mb-8">
            ...
            <p className="note-xs">{ui.frostbiteNote}</p>
          </section>
        )}
          ...
          <div className="flex flex-wrap gap-2">
            {near.map(o => (
              <Link prefetch={false}
                key={slugOf(o)}
                href={`${hub}/${slugOf(o)}`}
                className="rounded-xl border chip-off px-3 py-1.5 text-xs font-bold text-slate-600 dark:text-slate-300 tabular-nums hover:border-sky-500 hover:text-sky-700 dark:hover:text-sky-400 transition-colors"
              >
        ...
        <section className="mb-8">
          ...
          <p className="note-xs">{ui.windNote}</p>
        </section>
        ...
        <nav className="foot-nav" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link prefetch={false} key={l.lang} href={`${l.prefix}/windchill/${slug}`} hrefLang={l.hreflang} className="dim-link">
              {l.label}
            </Link>
          ))}
        </nav>
