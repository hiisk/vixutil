    ...
    <div className="page-wrap">
      ...
      <header className="page-head">
        <div className="page-bar">
          ...
          <span className="row-name">{f.size} · {c.mbps}Mbps</span>
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        ...
        <section className="mb-8">
          <h2 className="sec-h2-tight">{ui.bottleTitle}</h2>
          <p className="text-xs text-slate-400 dark:text-slate-500 mb-3 leading-relaxed">{ui.bottleNote}</p>
          {f.bottlenecks.length === 0 ? (
            <p className="rounded-2xl border chip-off px-4 py-3 text-sm text-slate-600 dark:text-slate-300">{ui.noneTag}</p>
          ) : (
            <ul className="list-card">
              {f.bottlenecks.map(b => (
                <li key={b.key} className="flex items-baseline justify-between gap-3 px-4 py-2.5">
                  ...
                  <span className="cell-num">{b.mbps}Mbps</span>
                </li>
              ))}
            </ul>
          )}
        ...
        <section className="mb-8">
          <h2 className="sec-h2-tight">{ui.streamTitle}</h2>
          ...
          <ul className="list-card">
            {f.streams.map(s => (
              <li key={s.key} className="flex items-baseline justify-between gap-3 px-4 py-2.5">
                ...
                <span className="cell-num">{s.count}</span>
              </li>
            ))}
          </ul>
        </section>
        ...
        <section className="mb-8">
          <h2 className="sec-h2">{ui.speedTitle}</h2>
        ...
        <nav className="foot-nav" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link prefetch={false} key={l.lang} href={`${l.prefix}/bandwidth/${slug}`} hrefLang={l.hreflang} className="dim-link">
              {l.label}
            </Link>
          ))}
        </nav>
