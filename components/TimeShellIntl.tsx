...
/**
 * 시간 도구 상세 페이지의 en/zh 공용 셸.
 *
 * 한국어 TimeShell은 SECTION_FAQ·CrossLinks·SiteFooter처럼 한국어에 묶인
 * 조각을 쓰고 있어 그대로 재사용하기 어렵다. 흐름은 같게 두고 별도 파일로 둔다 —
 * 한국어 경로를 건드리지 않는 편이 안전하다.
 *
 * h1은 여기서만 그린다. 도구 컴포넌트가 제목을 또 그리면 h1이 둘이 된다.
 */
export default function TimeShellIntl({
  slug,
  lang,
  children,
}: {
  ...
  const tool = findTimeToolIntl(lang, slug);
  if (!tool) throw new Error(`Unknown time tool: ${slug}`);
    ...
    <div className="page-wrap">
      <JsonLd
        data={breadcrumbJsonLd([
          ...
          { name: ui.section, path: `/${lang}/time` },
      ...
      <header className="page-head">
        <div className="page-bar">
          ...
          <Link href={`/${lang}/time`} className="text-sm text-slate-500 dark:text-slate-400 hover:text-sky-600 transition-colors font-medium">
            {ui.section}
          </Link>
          <span className="text-slate-200 dark:text-slate-700">·</span>
      ...
      <footer className="border-t border-slate-100 dark:border-slate-800 py-8 text-center">
        <Link href={`/${lang}`} className="text-sm font-bold text-sky-600">vixutil</Link>
      </footer>
    </div>
  );
}
