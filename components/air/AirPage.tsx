    ...
    <div className="page-wrap">
      ...
      <header className="page-head">
        <div className="page-bar">
          ...
          <span className="row-name tabular-nums">{ui.pollutantName(c.key)} {c.value}</span>
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        ...
        <section className="mb-8">
          ...
          <p className="note-xs">{ui.splitNote}</p>
        </section>

        {f.cigarettes !== null && (
          <section className="mb-8">
            <h2 className="sec-h2-tight">{ui.cigaretteTitle}</h2>
            <p className="note-xs">{ui.cigaretteNote}</p>
          </section>
        )}
        ...
        <nav className="foot-nav" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link prefetch={false} key={l.lang} href={`${l.prefix}/air/${slug}`} hrefLang={l.hreflang} className="dim-link">
              {l.label}
            </Link>
          ))}
        </nav>
