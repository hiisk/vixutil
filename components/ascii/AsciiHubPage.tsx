    ...
    <div className="page-wrap">
      ...
      <main id="main" className="relative max-w-2xl mx-auto px-4 py-8">
        ...
        {KINDS.map(kind => (
          <section key={kind} className="mb-8">
            <h2 className="sec-h2-tight">
              {ui.kindLabel[kind]}
              <span className="ml-1.5 text-[11px] font-bold text-slate-500 dark:text-slate-400">{codesOfKind(kind).length}</span>
            </h2>
            ...
            <div className="flex flex-wrap gap-1.5">
              {codesOfKind(kind).map(c => (
                <span
                  ...
                  <span className="tabular-nums text-slate-500 dark:text-slate-400">{c}</span> {asciiFacts(c).label}
                </span>
              ))}
            </div>
          </section>
        ...
        <section className="mb-8">
          <h2 className="sec-h2">
            {ui.groupTitle}
            <span className="ml-1.5 text-[11px] font-bold text-slate-500 dark:text-slate-400">{CONTROLS.length}</span>
          </h2>
          {GROUPS.map(g => (
            <div key={g} className="mb-4">
              <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200">{ui.groupLabel[g]}</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-2 leading-relaxed">{ui.groupNote[g]}</p>
              <div className="rounded-lg border border-slate-200 dark:border-slate-700 divide-y divide-slate-100 dark:divide-slate-800 overflow-hidden">
                {controlsOfGroup(g).map(x => (
                  <span
                    ...
                    <span className="text-[11px] text-slate-500 dark:text-slate-400 tabular-nums shrink-0 w-[26px] text-right">{x.code}</span>
                    ...
                    <span className="ml-auto text-[11px] font-mono text-slate-500 dark:text-slate-400 shrink-0">{asciiFacts(x.code).ctrl}</span>
                  </span>
                ))}
              </div>
            </div>
