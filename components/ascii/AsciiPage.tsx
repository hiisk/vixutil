      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
          ...
          <div className="flex flex-wrap gap-2">
            {neighbours(code).map(o => (
              <Link prefetch={false}
                key={o}
                href={`${hub}/${o}`}
                className="rounded-xl border chip-off px-3 py-1.5 text-xs font-bold text-slate-600 dark:text-slate-300 hover:border-teal-400 hover:text-teal-600 dark:hover:text-teal-400 transition-colors"
              >
        ...
        <nav className="foot-nav" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link prefetch={false} key={l.lang} href={`${l.prefix}/ascii/${code}`} hrefLang={l.hreflang} className="dim-link">
              {l.label}
            </Link>
          ))}
        </nav>
