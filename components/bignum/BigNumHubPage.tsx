...
import { BIGNUM_ICON, CELLS, UNITS, slugOf } from '@/lib/bignum/list';
import { atUnit, bigNumFacts } from '@/lib/bignum/facts';
import { BIGNUM_UI } from '@/lib/bignum/ui';

export default function BigNumHubPage({ lang }: { lang: Lang }) {
  const ui = BIGNUM_UI[lang];
  const prefix = langPrefix(lang);
  const homeHref = lang === 'ko' ? '/' : prefix || '/';
  const path = `${prefix}/bignum`;
  const base = localeOfLang(lang);
  const notes: [string, string][] = [
    [ui.groupTitle, ui.groupNote],
    [ui.indianTitle, ui.indianNote],
    [ui.eastTitle, ui.eastNote],
  ...
  return (
    <div className="relative min-h-screen bg-white dark:bg-slate-900">
      ...
      />
      <JsonLd
        data={itemListJsonLd(ui.hubTitle, path, CELLS.map(c => ({ name: `${c.factor} ${ui.unitName(c.unit)}`, path: `${path}/${slugOf(c)}` })))}
      />
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl mb-3 shadow-lg bg-gradient-to-br from-cyan-600 to-sky-400">
        ...
        <section className="mb-8">
          <h2 className="text-base font-black text-slate-800 dark:text-slate-100 mb-3">1 {ui.unitName('crore')}</h2>
        ...
        {UNITS.map(u => (
          <section key={u.key} className="mb-6">
            <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200 mb-2">
              ...
              <span className="ml-2 text-xs font-normal text-slate-400 dark:text-slate-500 tabular-nums">
                10^{u.exp} · {ui.systemName(u.system)}
              </span>
            </h3>
            <BigNumList cells={atUnit(u.key)} path={path} name={ui.unitName} />
          </section>
        ))}
        ...
        <nav className="mt-8 flex flex-wrap justify-center gap-x-3 gap-y-1.5 text-xs font-bold text-slate-400 dark:text-slate-500" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link key={l.lang} href={`${l.prefix}/bignum`} hrefLang={l.hreflang} className="hover:text-slate-700 dark:hover:text-slate-300 transition-colors">
              {l.label}
            </Link>
          ))}
        </nav>
