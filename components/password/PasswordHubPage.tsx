    ...
    <div className="relative min-h-screen bg-white dark:bg-slate-900">
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl mb-3 shadow-lg bg-gradient-to-br from-teal-600 to-emerald-400">
            <ToolIcon emoji={PASSWORD_ICON} accent="rgba(255,255,255,0.55)" className="w-7 h-7 text-white" />
          </div>
        ...
        <section className="mb-4 mt-8">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 leading-relaxed">{ui.sizeLabel} · {ui.lengthLabel}</p>
        </section>
        ...
        <nav className="foot-nav" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link key={l.lang} href={`${l.prefix}/password`} hrefLang={l.hreflang} className="hover:text-slate-700 dark:hover:text-slate-300 transition-colors">
              {l.label}
            </Link>
          ))}
        </nav>
