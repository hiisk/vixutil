    ...
    <div className="page-wrap">
      ...
      <header className="page-head">
        <div className="max-w-3xl mx-auto px-4 h-14 flex items-center gap-2">
          <Link prefetch={false} href={homeHref} className="page-back hover:text-slate-600 dark:text-slate-300 dark:hover:text-slate-300">
      ...
      <main id="main" className="relative max-w-3xl mx-auto px-4 py-9">
        ...
        <nav className="foot-nav">
          {LEGAL_KINDS.filter(k => k !== kind).map(k => (
            <Link prefetch={false} key={k} href={localeHref(locale, legalRoute(k))} className="hover:text-slate-600 dark:text-slate-300 dark:hover:text-slate-300">
              {chrome.nav[k]}
            </Link>
          ))}
        </nav>
