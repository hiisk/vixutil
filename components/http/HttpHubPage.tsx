 ...
 */
export default function HttpHubPage({ lang }: { lang: Lang }) {
      ...
      <header className="page-head">
        <div className="page-bar">
          ...
          <div className="ml-auto shrink-0">
            <LangPicker current={localeOfLang(lang)} route={`/http`} available={LOCALE_PATHS} />
          </div>
        </div>
      </header>
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl mb-3 shadow-lg bg-sec-soft">
            <ToolIcon emoji={HTTP_ICON} className="w-7 h-7" />
          </div>
        ...
        <nav className="foot-nav" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link prefetch={false} key={l.lang} href={`${l.prefix}/http`} hrefLang={l.hreflang} className="dim-link">
              {l.label}
            </Link>
          ))}
        </nav>
