...
export default function CopyButton({ text, copy, copied }: { text: string; copy: string; copied: string }) {
  ...
  return (
    <button
      onClick={onClick}
      className="text-xs font-bold px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-blue-300 hover:text-blue-600 transition-colors"
    >
      {done ? copied : copy}
    </button>
  );
