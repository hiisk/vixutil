    ...
    <div className="page-wrap">
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        ...
        <section className="mb-8">
          <h2 className="sec-h2-tight">{ui.weightLabel} 70 kg</h2>
        ...
        <section className="mb-8">
          <h2 className="sec-h2-tight">{ui.heavyTitle}</h2>
          <p className="note-xs">{ui.heavyNote}</p>
        </section>

        <section className="mb-8">
          <h2 className="sec-h2-tight">{ui.jumpTitle}</h2>
          <p className="note-xs">{ui.jumpNote}</p>
        </section>

        <section className="mb-8">
          <h2 className="sec-h2-tight">{ui.gasTitle}</h2>
          <p className="note-xs">{ui.gasNote}</p>
        </section>
          ...
          <div className="flex flex-wrap gap-1">
            {WEIGHTS.map(w => (
              <Link prefetch={false}
                key={w}
                href={`${path}/${w}`}
                className="w-12 rounded-md border chip-off py-1 text-center text-[11px] font-bold text-slate-600 dark:text-slate-300 tabular-nums hover:border-indigo-500 hover:text-indigo-700 dark:hover:text-indigo-400 transition-colors"
              >
                {w}
              </Link>
            ))}
        ...
        <nav className="foot-nav" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link prefetch={false} key={l.lang} href={`${l.prefix}/gravity`} hrefLang={l.hreflang} className="dim-link">
              {l.label}
            </Link>
          ))}
        </nav>
