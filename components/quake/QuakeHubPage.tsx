...
import { MAGNITUDES, QUAKE_ICON, slugOf } from '@/lib/quake/list';
import { quakeFacts } from '@/lib/quake/facts';
import { QUAKE_UI } from '@/lib/quake/ui';

/** 정수 규모만 뽑아 큰 그림을 먼저 보인다 */
const WHOLE = [5, 6, 7, 8, 9];

export default function QuakeHubPage({ lang }: { lang: Lang }) {
  const ui = QUAKE_UI[lang];
  const prefix = langPrefix(lang);
  const homeHref = lang === 'ko' ? '/' : prefix || '/';
  const path = `${prefix}/quake`;
  const base = localeOfLang(lang);
  const notes: [string, string][] = [
    [ui.logTitle, ui.logNote],
    [ui.amplitudeTitle, ui.amplitudeNote],
    [ui.compareTitle, ui.compareNote],
    [ui.intensityTitle, ui.intensityNote],
  ...
  return (
    <div className="relative min-h-screen bg-white dark:bg-slate-900">
      ...
      />
      <JsonLd
        data={itemListJsonLd(ui.hubTitle, path, MAGNITUDES.map(m => ({ name: `M ${m.toFixed(2)}`, path: `${path}/${slugOf(m)}` })))}
      />
      ...
      <header className="bg-white/90 dark:bg-slate-900/90 backdrop-blur border-b border-slate-100 dark:border-slate-800 sticky top-0 z-20">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-2">
          ...
          <div className="ml-auto shrink-0">
            <LangPicker current={localeOfLang(lang)} route="/quake" available={LOCALE_PATHS} />
          </div>
        </div>
      </header>
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl mb-3 shadow-lg bg-gradient-to-br from-rose-800 to-orange-400">
            <ToolIcon emoji={QUAKE_ICON} accent="rgba(255,255,255,0.55)" className="w-7 h-7 text-white" />
          </div>
        ...
        <section className="mb-8">
          <h2 className="text-base font-black text-slate-800 dark:text-slate-100 mb-3">{ui.hiroshimaLabel}</h2>
        ...
        <section className="mb-4 mt-8">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 leading-relaxed">{ui.tntLabel}</p>
        </section>

        {WHOLE.map(whole => (
          <section key={whole} className="mb-6">
            <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200 mb-2 tabular-nums">
              M {whole.toFixed(1)} – {(whole + 0.95).toFixed(2)}
            </h3>
            <QuakeList magnitudes={MAGNITUDES.filter(m => m >= whole && m < whole + 1)} path={path} />
          </section>
        ))}

        <section className="mb-6">
          <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200 mb-2 tabular-nums">M 4.00 – 4.95</h3>
          <QuakeList magnitudes={MAGNITUDES.filter(m => m < 5)} path={path} />
        </section>
        ...
        <nav className="mt-8 flex flex-wrap justify-center gap-x-3 gap-y-1.5 text-xs font-bold text-slate-400 dark:text-slate-500" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link key={l.lang} href={`${l.prefix}/quake`} hrefLang={l.hreflang} className="hover:text-slate-700 dark:hover:text-slate-300 transition-colors">
              {l.label}
            </Link>
          ))}
        </nav>
