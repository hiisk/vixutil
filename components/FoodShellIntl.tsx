...
import { findFoodToolIntl, relatedFoodToolsIntl, FOOD_SHELL_UI, type FoodIntlLang } from '@/lib/food-tools-intl';

/**
 * 요리 도구 상세 페이지의 en/zh 공용 셸. TimeShellIntl과 같은 구조다.
 *
 * 한국어 FoodShell은 SECTION_FAQ·CrossLinks처럼 한국어에 묶인 조각을 쓰고 있어
 * 그대로 재사용하기 어렵다. 흐름은 같게 두고 별도 파일로 둔다.
 *
 * h1은 여기서만 그린다. 도구 컴포넌트가 제목을 또 그리면 h1이 둘이 된다.
 */
export default function FoodShellIntl({
  slug,
  lang,
  children,
}: {
  ...
  const tool = findFoodToolIntl(lang, slug);
  if (!tool) throw new Error(`Unknown food tool: ${slug}`);

  const ui = FOOD_SHELL_UI[lang];
  const path = `/${lang}/food/${tool.slug}`;
  const related = relatedFoodToolsIntl(lang, tool.slug);
    ...
    <div className="relative min-h-screen bg-white dark:bg-slate-900">
      <JsonLd
        data={breadcrumbJsonLd([
          ...
          { name: ui.section, path: `/${lang}/food` },
      ...
      <header className="page-head">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-2">
          <Link href={`/${lang}`} className="flex items-center gap-1.5 text-sm text-slate-400 dark:text-slate-500 hover:text-amber-600 transition-colors font-medium">
          ...
          <Link href={`/${lang}/food`} className="text-sm text-slate-400 dark:text-slate-500 hover:text-amber-600 transition-colors font-medium">
            {ui.section}
          </Link>
          <span className="text-slate-200 dark:text-slate-700">·</span>
          <span className="text-sm font-semibold text-slate-700 dark:text-slate-200 truncate">{tool.title}</span>
          <span className="ml-auto shrink-0">
            <LangPicker current={lang} route={`/food/${slug}`} />
          </span>
        </div>
      </header>
        ...
        <section className="mt-8" aria-label={ui.canDo}>
          ...
          <ul className="grid sm:grid-cols-2 gap-2">
            {tool.features.map(f => (
              ...
              >
                <svg aria-hidden="true" className="w-4 h-4 mt-0.5 shrink-0 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
        ...
        <section className="mt-8" aria-label={ui.others}>
          ...
          <div className="grid sm:grid-cols-2 gap-2">
            {related.map(r => (
              <Link
                key={r.slug}
                href={`/${lang}/food/${r.slug}`}
                className="group hub-card hover:border-amber-300 hover:shadow-sm transition-all"
              >
                <ToolIcon emoji={r.icon} color={r.og[0]} accent={r.og[1]} className="w-5 h-5 shrink-0" />
                <span className="hub-card-body">
                  <span className="hub-card-title group-hover:text-amber-700 transition-colors">
                    {r.title}
                  </span>
      ...
      <footer className="border-t border-slate-100 dark:border-slate-800 py-8 text-center">
        <Link href={`/${lang}`} className="text-sm font-black text-amber-600">vixutil</Link>
      </footer>
    </div>
  );
}
