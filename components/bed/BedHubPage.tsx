...
import { BEDS, BED_ICON, CELLS, slugOf } from '@/lib/bed/list';
import { atBed, bedFacts } from '@/lib/bed/facts';
import { BED_UI } from '@/lib/bed/ui';

export default function BedHubPage({ lang }: { lang: Lang }) {
  const ui = BED_UI[lang];
  const prefix = langPrefix(lang);
  const homeHref = lang === 'ko' ? '/' : prefix || '/';
  const path = `${prefix}/bed`;
  const base = localeOfLang(lang);
  const notes: [string, string][] = [
    [ui.nameTitle, ui.nameNote],
    [ui.walkTitle, ui.walkNote],
    [ui.personTitle, ui.personNote],
    [ui.ksTitle, ui.ksNote],
    [ui.careTitle, ui.careNote],
  ];
  const inRoom = BEDS.map(b => bedFacts({ bed: b.key, room: 3600 }));
  ...
  return (
    <div className="relative min-h-screen bg-white dark:bg-slate-900">
      ...
      />
      <JsonLd
        data={itemListJsonLd(ui.hubTitle, path, CELLS.map(c => ({ name: `${ui.bedName(c.bed)} ${c.room}`, path: `${path}/${slugOf(c)}` })))}
      />
      ...
      <header className="bg-white/90 dark:bg-slate-900/90 backdrop-blur border-b border-slate-100 dark:border-slate-800 sticky top-0 z-20">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-2">
          ...
          <div className="ml-auto shrink-0">
            <LangPicker current={localeOfLang(lang)} route="/bed" available={LOCALE_PATHS} />
          </div>
        </div>
      </header>
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl mb-3 shadow-lg bg-gradient-to-br from-violet-700 to-fuchsia-400">
            <ToolIcon emoji={BED_ICON} accent="rgba(255,255,255,0.55)" className="w-7 h-7 text-white" />
          </div>
        ...
        <section className="mb-8">
          <h2 className="text-base font-black text-slate-800 dark:text-slate-100 mb-3">3600 mm</h2>
          <ul className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 divide-y divide-slate-100 dark:divide-slate-800">
            {inRoom.map(f => (
              <li key={f.slug} className="flex items-baseline justify-between gap-3 px-4 py-2.5">
                <span className="text-sm text-slate-600 dark:text-slate-300 truncate">{ui.bedName(f.cell.bed)}</span>
        ...
        <section className="mb-4 mt-8">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 leading-relaxed">{ui.gapLabel}</p>
        </section>
        ...
        {BEDS.map(b => (
          <section key={b.key} className="mb-6">
            ...
            <BedList cells={atBed(b.key)} path={path} name={ui.bedName} />
          </section>
        ))}
        ...
        <nav className="mt-8 flex flex-wrap justify-center gap-x-3 gap-y-1.5 text-xs font-bold text-slate-400 dark:text-slate-500" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link key={l.lang} href={`${l.prefix}/bed`} hrefLang={l.hreflang} className="hover:text-slate-700 dark:hover:text-slate-300 transition-colors">
              {l.label}
            </Link>
          ))}
        </nav>
