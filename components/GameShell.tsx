...
export default function GameShell({
  ...
  const tool = findGameTool(slug);
  if (!tool) throw new Error(`알 수 없는 두뇌 게임 도구: ${slug}`);

  const path = `/game/${tool.slug}`;
  const related = relatedGameTools(tool.slug);
    ...
    <div className="relative min-h-screen bg-white dark:bg-slate-900">
      ...
      <header className="page-head">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-2">
          ...
          <Link href="/game" className="text-sm text-slate-400 dark:text-slate-500 hover:text-emerald-600 transition-colors font-medium">
            두뇌 게임
          </Link>
          <span className="text-slate-200 dark:text-slate-700">·</span>
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        ...
        <Faq items={SECTION_FAQ[`game/${tool.slug}`]} />
