...
import { findTextToolIntl, relatedTextToolsIntl, TEXT_SHELL_UI, type TextIntlLang } from '@/lib/text-tools-intl';

/**
 * 텍스트 도구 상세 페이지의 en/zh 공용 셸. TimeShellIntl과 같은 구조다.
 *
 * 한국어 TextShell은 SECTION_FAQ·CrossLinks처럼 한국어에 묶인 조각을 쓰고 있어
 * 그대로 재사용하기 어렵다. 흐름은 같게 두고 별도 파일로 둔다.
 *
 * h1은 여기서만 그린다. 도구 컴포넌트가 제목을 또 그리면 h1이 둘이 된다.
 */
export default function TextShellIntl({
  slug,
  lang,
  children,
}: {
  ...
  const tool = findTextToolIntl(lang, slug);
  if (!tool) throw new Error(`Unknown text tool: ${slug}`);
    ...
    <div className="relative min-h-screen bg-white dark:bg-slate-900">
      <JsonLd
        data={breadcrumbJsonLd([
          ...
          { name: ui.section, path: `/${lang}/text` },
      ...
      <header className="bg-white/90 dark:bg-slate-900/90 backdrop-blur border-b border-slate-100 dark:border-slate-800 sticky top-0 z-20">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-2">
          ...
          <span className="ml-auto shrink-0">
            <LangPicker current={lang} route={`/text/${slug}`} />
          </span>
        </div>
      </header>
