...
export default function CrossLinks({ className = 'mt-8' }: { className?: string }) {
  ...
  return (
    <section className={className} aria-label="함께 보면 좋은 콘텐츠">
      <h2 className="text-base font-black text-slate-800 mb-3">함께 보면 좋아요</h2>
        ...
        {links.map(l => (
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-bold text-slate-800 group-hover:text-indigo-700 transition-colors">
                {l.title}
              </span>
              <span className="block text-xs text-slate-500 leading-relaxed mt-0.5">{l.why}</span>
            </span>
            <svg className="w-4 h-4 text-slate-300 shrink-0 mt-2.5 group-hover:text-indigo-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
