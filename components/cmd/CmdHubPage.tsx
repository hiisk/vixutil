    ...
    <div className="page-wrap">
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        <div className="text-center mb-7">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl mb-4 shadow-lg bg-gradient-to-br from-slate-700 to-indigo-500">
            <ToolIcon emoji={CMD_ICON} accent="rgba(255,255,255,0.55)" className="w-8 h-8 text-white" />
          </div>
              ...
              <div className="grid sm:grid-cols-2 gap-2">
                {list.map(x => (
                  <Link
                    key={x.slug}
                    href={`${path}/${x.slug}`}
                    className="group rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 hover:border-indigo-300 hover:shadow-sm transition-all"
                  >
        ...
        <nav className="foot-nav" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link key={l.lang} href={`${l.prefix}/cmd`} hrefLang={l.hreflang} className="hover:text-slate-700 dark:hover:text-slate-300 transition-colors">
              {l.label}
            </Link>
          ))}
        </nav>
