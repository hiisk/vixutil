    ...
    <div>
      ...
      {tab === 'coin' && (
        <div className="text-center">
          <div className={`mx-auto mb-6 flex items-center justify-center w-32 h-32 rounded-full bg-sec-soft text-4xl font-bold shadow-sm ${flipping ? 'coin-spin' : ''}`}>
            {flipping || !coinFace ? '🪙' : coinFace}
          </div>
          {coinFace && !flipping && (
            <div className="wc-pop text-2xl font-bold text-slate-800 dark:text-slate-100 mb-6">{ui.coinResult(coinFace)}</div>
          )}
          <button onClick={flip} disabled={flipping} className="w-full bg-sec font-bold text-lg rounded-lg py-4 hover:-translate-y-0.5 hover:border-slate-300 dark:hover:border-slate-700 transition-all disabled:opacity-60">
      ...
      {tab === 'dice' && (
        <div className="text-center">
          ...
          <button onClick={roll} disabled={rolling} className="w-full bg-sec font-bold text-lg rounded-lg py-4 hover:-translate-y-0.5 hover:border-slate-300 dark:hover:border-slate-700 transition-all disabled:opacity-60">
