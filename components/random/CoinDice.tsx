...
export default function CoinDice() {
  ...
  // 동전
  const [coin, setCoin] = useState<'앞' | '뒤' | null>(null);
    ...
    <div>
      ...
      {tab === 'coin' && (
        <div className="text-center">
          <div className={`mx-auto mb-6 flex items-center justify-center w-32 h-32 rounded-full bg-gradient-to-br from-amber-300 to-yellow-500 text-white text-5xl font-black shadow-lg ${flipping ? 'coin-spin' : ''}`}>
            {flipping ? '🪙' : coin ?? '🪙'}
          </div>
          {coin && !flipping && (
            <div className="wc-pop text-2xl font-black text-slate-800 dark:text-slate-100 mb-6">{coin}면!</div>
          )}
      ...
      {tab === 'dice' && (
        <div className="text-center">
          <div className="flex items-center justify-center gap-2 mb-5">
            <span className="text-sm font-bold text-slate-500 dark:text-slate-400">주사위 개수</span>
            {[1, 2, 3, 4, 5].map(n => (
          ...
          {dice && !rolling && dice.length > 1 && (
            <div className="text-sm font-bold text-slate-500 dark:text-slate-400 mb-4">합계 {dice.reduce((a, b) => a + b, 0)}</div>
          )}
