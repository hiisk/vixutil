...
const TEAM_NAMES = ['1팀', '2팀', '3팀', '4팀', '5팀', '6팀'];
...
export default function TeamMaker() {
  ...
  return (
    <div>
      ...
      />
      <div className="mt-2 mb-4 text-xs text-slate-400">총 {members.length}명</div>

      <div className="flex items-center gap-2 mb-5">
        <span className="text-sm font-bold text-slate-500 dark:text-slate-400">팀 개수</span>
        <input
        ...
        <div className="grid grid-cols-2 gap-3">
          {teams.map((team, i) => (
            <div key={i} className={`wc-pop rounded-2xl bg-gradient-to-br ${TEAM_COLORS[i % TEAM_COLORS.length]} text-white p-4`} style={{ animationDelay: `${i * 70}ms` }}>
              <div className="text-xs font-bold text-white/80 mb-2">{TEAM_NAMES[i]} · {team.length}명</div>
