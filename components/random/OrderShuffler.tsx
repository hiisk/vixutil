  ...
  return (
    <div>
      ...
      <button
        ...
        className="w-full bg-sec font-bold text-lg rounded-lg py-4 mb-6 shadow-sm shadow-cyan-200 dark:shadow-none hover:-translate-y-0.5 hover:border-slate-300 dark:hover:border-slate-700 transition-all disabled:opacity-50"
      >
        {ui.shuffleOrder}
      </button>
      ...
      {order && (
        <ol className="space-y-2">
          {order.map((item, i) => (
            ...
            >
              <span className="flex items-center justify-center w-8 h-8 shrink-0 rounded-full bg-sec-soft text-sm font-bold">{i + 1}</span>
