...
import { TOPIC_OF_DOMAIN, TOPIC_DOMAIN, topicEvidence, topicQuery, type TopicSlug } from '@/lib/saju-topics';
...
export default function SajuKo({ initialTopic, formExtra, topicHead, topicTail, faq }: {
  /** 주제 낱장에서 왔으면 그 영역 단계를 열고 시작한다 */
  initialTopic?: TopicSlug;
  /** 이름 칸처럼 그 화면만의 입력 — 폼 안에 들어간다 */
  formExtra?: ReactNode;
  /** 폼 위 — 주제 낱장의 제목·소개 */
  topicHead?: ReactNode;
  /** 맨 아래 — 주제 낱장의 배경 설명 */
  ...
  /* 사용자가 단계를 한 번이라도 옮겼나 — 옮기기 전에는 주제가 자리를 정한다 */
   ...
   *
   * effect로 setStepIdx를 부르지 않는다 — 그러면 렌더가 한 번 더 돌고, 이 저장소의
   * 린트 규칙도 막는다. 대신 **손대기 전까지는 주제가 정한 자리**를 쓴다.
   * 사용자가 단계를 한 번이라도 옮기면 그 뒤로는 사용자 값만 본다.
   */
  const topicStep = initialTopic
    ? allSteps.findIndex(s2 => s2.key === `domain-${TOPIC_DOMAIN[initialTopic]}`)
    : -1;
  const safeStep = touched || topicStep < 0
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <div className="max-w-xl mx-auto px-4 py-6 pb-16 space-y-4">
        ...
        */}
        <div className="pt-2">
          <SajuTopicNav lang="ko" current={initialTopic} query={topicQuery(form)} />
        </div>
