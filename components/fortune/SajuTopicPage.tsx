        ...
        >
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            {(['year', 'month', 'day'] as const).map(k => (
              <input key={k} type="number" inputMode="numeric" value={form[k]}
                placeholder={k === 'year' ? '1995' : k === 'month' ? '1-12' : '1-31'}
          ...
          <input type="time" value={form.hour} onChange={e => setForm(f => ({ ...f, hour: e.target.value }))}
            className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2.5 text-sm text-slate-800 dark:text-slate-100 focus:border-indigo-400 focus:outline-none mb-3" />
