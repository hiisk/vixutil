    ...
    <div className="relative min-h-screen bg-white dark:bg-slate-900">
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        ...
        <section className="mb-8 mt-8">
          <h2 className="text-base font-black text-slate-800 dark:text-slate-100 mb-1">{ui.wattLabel}</h2>
        ...
        <section className="mb-4">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 leading-relaxed">{ui.hubLead}</p>
        </section>
        ...
        <nav className="mt-8 flex flex-wrap justify-center gap-x-3 gap-y-1.5 text-xs font-bold text-slate-400 dark:text-slate-500" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link key={l.lang} href={`${l.prefix}/battery`} hrefLang={l.hreflang} className="hover:text-slate-700 dark:hover:text-slate-300 transition-colors">
              {l.label}
            </Link>
          ))}
        </nav>
