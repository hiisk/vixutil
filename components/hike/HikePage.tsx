    ...
    <div className="page-wrap">
      ...
      <header className="page-head">
        <div className="page-bar">
          ...
          <span className="row-name tabular-nums">{c.km}km · ↑{c.up}m</span>
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        ...
        <section className="mb-8">
          <h2 className="sec-h2-tight">{ui.downTitle}</h2>
          <p className="note-xs">{ui.downNote}</p>
        </section>
        ...
        <section className="mb-8">
          <h2 className="sec-h2">{ui.ascentRowTitle}</h2>
          <HikeList cells={atDistance(c.km)} path={hub} clock={ui.clock} current={slug} />
        </section>

        <section className="mb-8">
          <h2 className="sec-h2">{ui.distanceRowTitle}</h2>
        ...
        <nav className="foot-nav" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link prefetch={false} key={l.lang} href={`${l.prefix}/hike/${slug}`} hrefLang={l.hreflang} className="dim-link">
              {l.label}
            </Link>
          ))}
        </nav>
