...
import { ASCENTS, CELLS, DISTANCES, HIKE_ICON, slugOf } from '@/lib/hike/list';
import { atDistance, flatPerMetre, hikeFacts } from '@/lib/hike/facts';
import { HIKE_UI } from '@/lib/hike/ui';

export default function HikeHubPage({ lang }: { lang: Lang }) {
  const ui = HIKE_UI[lang];
  const prefix = langPrefix(lang);
  const homeHref = lang === 'ko' ? '/' : prefix || '/';
  const path = `${prefix}/hike`;
  const base = localeOfLang(lang);
  const notes: [string, string][] = [
    [ui.ruleTitle, ui.ruleNote],
    [ui.equalTitle, ui.equalNote],
    [ui.downTitle, ui.downNote],
  ...
  return (
    <div className="relative min-h-screen bg-white dark:bg-slate-900">
      ...
      />
      <JsonLd
        data={itemListJsonLd(ui.hubTitle, path, CELLS.map(c => ({ name: `${c.km}km · ${c.up}m`, path: `${path}/${slugOf(c)}` })))}
      />
      ...
      <header className="bg-white/90 dark:bg-slate-900/90 backdrop-blur border-b border-slate-100 dark:border-slate-800 sticky top-0 z-20">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-2">
          ...
          <div className="ml-auto shrink-0">
            <LangPicker current={localeOfLang(lang)} route="/hike" available={LOCALE_PATHS} />
          </div>
        </div>
      </header>
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl mb-3 shadow-lg bg-gradient-to-br from-green-700 to-lime-500">
            <ToolIcon emoji={HIKE_ICON} accent="rgba(255,255,255,0.55)" className="w-7 h-7 text-white" />
          </div>
        ...
        <section className="mb-8 mt-8">
          <h2 className="text-base font-black text-slate-800 dark:text-slate-100 mb-3">{ui.ascentLabel}</h2>
        ...
        <section className="mb-4">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 leading-relaxed">{ui.upLabel}</p>
        </section>

        {DISTANCES.map(km => (
          <section key={km} className="mb-6">
            <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200 mb-2 tabular-nums">
              {km} km
              <span className="ml-2 text-xs font-normal text-slate-400 dark:text-slate-500">
                {ui.gradeName(hikeFacts({ km, up: 500 }).grade)}
              </span>
            </h3>
            <HikeList cells={atDistance(km)} path={path} clock={ui.clock} />
          </section>
        ))}
        ...
        <nav className="mt-8 flex flex-wrap justify-center gap-x-3 gap-y-1.5 text-xs font-bold text-slate-400 dark:text-slate-500" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link key={l.lang} href={`${l.prefix}/hike`} hrefLang={l.hreflang} className="hover:text-slate-700 dark:hover:text-slate-300 transition-colors">
              {l.label}
            </Link>
          ))}
        </nav>
