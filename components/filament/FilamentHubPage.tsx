...
import { CELLS, FILAMENT_ICON, MATERIALS, slugOf } from '@/lib/filament/list';
import { atMaterial } from '@/lib/filament/facts';
import { FILAMENT_UI, kgOf, materialName } from '@/lib/filament/ui';

export default function FilamentHubPage({ lang }: { lang: Lang }) {
  const ui = FILAMENT_UI[lang];
  const prefix = langPrefix(lang);
  const homeHref = lang === 'ko' ? '/' : prefix || '/';
  const path = `${prefix}/filament`;
  const base = localeOfLang(lang);
  const notes: [string, string][] = [
    [ui.densityTitle, ui.densityNote],
    [ui.diameterTitle, ui.diameterNote],
    [ui.netTitle, ui.netNote],
    [ui.remainTitle, ui.remainNote],
  ];
  ...
  return (
    <div className="page-wrap">
      ...
      />
      <JsonLd
        data={itemListJsonLd(ui.hubTitle, path, CELLS.map(c => ({ name: `${materialName(c.material)} ${kgOf(c.grams)}`, path: `${path}/${slugOf(c)}` })))}
      />
      ...
      <header className="page-head">
        <div className="page-bar">
          ...
          <div className="ml-auto shrink-0">
            <LangPicker current={localeOfLang(lang)} route="/filament" available={LOCALE_PATHS} />
          </div>
        </div>
      </header>
      ...
      <main className="relative max-w-2xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl mb-3 shadow-lg bg-gradient-to-br from-violet-500 to-fuchsia-400">
            <ToolIcon emoji={FILAMENT_ICON} accent="rgba(255,255,255,0.55)" className="w-7 h-7 text-white" />
          </div>
        ...
        <section className="mb-4 mt-8">
          ...
          <p className="note-xs">{ui.lengthLabel}</p>
        </section>
          ...
          <section key={m.key} className="mb-6">
            <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200 mb-2 tabular-nums">
              ...
              <span className="ml-2 text-xs font-normal text-slate-400 dark:text-slate-500 tabular-nums">
                {ui.densityLabel} {m.density} g/cm³
              </span>
            </h3>
            <FilamentList cells={atMaterial(m.key)} path={path} by="spool" />
          </section>
        ))}
        ...
        <nav className="foot-nav" aria-label="Language">
          {LANGS.filter(l => l.lang !== lang).map(l => (
            <Link key={l.lang} href={`${l.prefix}/filament`} hrefLang={l.hreflang} className="hover:text-slate-700 dark:hover:text-slate-300 transition-colors">
              {l.label}
            </Link>
          ))}
        </nav>
