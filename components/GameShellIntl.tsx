...
/**
 * 두뇌 게임 상세 페이지의 en/zh 공용 셸. TimeShellIntl과 같은 구조다.
 *
 * 한국어 GameShell은 SECTION_FAQ·CrossLinks처럼 한국어에 묶인 조각을 쓰고 있어
 * 그대로 재사용하기 어렵다. 흐름은 같게 두고 별도 파일로 둔다.
 *
 * h1은 여기서만 그린다. 도구 컴포넌트가 제목을 또 그리면 h1이 둘이 된다.
 */
export default function GameShellIntl({
  slug,
  lang,
  children,
}: {
  ...
  const tool = findGameToolIntl(lang, slug);
  if (!tool) throw new Error(`Unknown game: ${slug}`);
    ...
    <div className="relative min-h-screen bg-white dark:bg-slate-900">
      <JsonLd
        data={breadcrumbJsonLd([
          ...
          { name: ui.section, path: `/${lang}/game` },
      ...
      <header className="bg-white/90 dark:bg-slate-900/90 backdrop-blur border-b border-slate-100 dark:border-slate-800 sticky top-0 z-20">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-2">
          <Link href={`/${lang}`} className="flex items-center gap-1.5 text-sm text-slate-400 dark:text-slate-500 hover:text-emerald-600 transition-colors font-medium">
          ...
          <Link href={`/${lang}/game`} className="text-sm text-slate-400 dark:text-slate-500 hover:text-emerald-600 transition-colors font-medium">
            {ui.section}
          </Link>
          <span className="text-slate-200 dark:text-slate-700">·</span>
        ...
        <section className="mt-8" aria-label={ui.canDo}>
          ...
          <ul className="grid sm:grid-cols-2 gap-2">
            {tool.features.map(f => (
              ...
              >
                <svg aria-hidden="true" className="w-4 h-4 mt-0.5 shrink-0 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
        ...
        <section className="mt-8" aria-label={ui.others}>
          ...
          <div className="grid sm:grid-cols-2 gap-2">
            {related.map(r => (
              <Link
                key={r.slug}
                href={`/${lang}/game/${r.slug}`}
                className="group flex items-center gap-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 hover:border-emerald-300 hover:shadow-sm transition-all"
              >
                <span className="text-xl shrink-0">{r.icon}</span>
                <span className="min-w-0 flex-1">
                  <span className="block text-sm font-bold text-slate-800 dark:text-slate-100 group-hover:text-emerald-700 transition-colors">
                    {r.title}
                  </span>
      ...
      <footer className="border-t border-slate-100 dark:border-slate-800 py-8 text-center">
        ...
        <p className="text-xs text-slate-400 mt-1">
          ...
          {' · '}
          <Link href={`/${other}/time/${slug}`} className="hover:text-emerald-600" hrefLang={other}>
            {other === 'zh' ? '中文' : 'EN'}
          </Link>
        </p>
      </footer>
