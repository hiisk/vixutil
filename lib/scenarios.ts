 ...
 * 아무 모델도 가정하지 않으므로 실제로 흔들린다(BTC 30일 기준 29걸음 중 7번 하락).
 *
 * 다만 그 흔들림은 **구조가 아니라 표본 잡음이다**. 같은 길이·같은 drift·같은 변동성·같은
 * 팻테일을 가진 랜덤워크를 400회 시뮬레이션해 동일한 방식으로 경로를 만들면:
 *   BTC 실측 하락 7걸음 -> 랜덤워크 중앙값 6, P(>=7) = 47%
 *   ETH 실측 14걸음     -> 중앙값 14,        P(>=14) = 55%
 *   SOL 실측 14걸음     -> 중앙값 11,        P(>=14) = 33%
 * 아무 구조 없는 랜덤워크가 절반 가까운 확률로 같은 모양을 만든다.
 *
 * 따라서 이 경로는 **수준(level)** 을 읽는 데 쓰고, **굴곡의 모양**을 신호로 읽어서는 안 된다.
 * 예측선이 이 흔들림을 재현하면 그것은 잡음을 적합하는 것이다. UI에도 그렇게 고지한다.
 */
