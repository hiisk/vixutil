...
export interface TopicCopy {
  ...
  /**
   * 주제마다 다른 배경 해설 — 「이 운은 사주에서 무엇을 보는가」.
   *
   * ── 왜 넣었나 (2026-08-19) ──────────────────────────────
   * 주제 일곱이 제목과 소개 한 줄만 다르고 **본문이 같았다.** 통합 화면을 그대로
   * 띄우기로 하면서 근거·배경 문단이 빠졌기 때문이다. 그러면 「사주 연애운」으로
   * 들어온 사람과 「사주 승진운」으로 들어온 사람이 똑같은 글을 읽는다 —
   * 검색엔진 쪽에서도 주소 일곱이 서로의 사본으로 보인다.
   *
   * 여기 적는 것은 그 사람의 사주가 아니라 **명리가 그 주제에서 무엇을 보는가**다.
   * 사주에 따라 달라지는 말은 lib/saju-fortune.ts가 이미 내고 있으므로 겹치지 않는다.
   */
  body: Record<TopicSlug, { h: string; p: string }[]>;
  ui: {
    /** 명식을 뽑기 전 안내 */
    empty: string;
    /** 근거 블록 제목 */
