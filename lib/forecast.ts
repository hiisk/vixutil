 ...
 *
 * 경쟁 사이트(CoinCodex)가 밝힌 방법 — "moving averages, RSI and MACD" — 도 그대로 구현해
 * 검정했다. 5일 전방, 비중첩 구간, 25개 코인:
 *
 *                          시장성분 미제거          시장성분 제거(market-neutral)
 *   지표              coin-level t   적중률     coin-level t  적중률  반기 t
 *   MA(20/50/100/200)     2.41       51.0%         1.66       50.5%  1.03 / 2.18
 *   RSI(14)               2.87       49.9%         1.26       48.9%  0.70 / 1.81
 *   MACD(12,26,9)         2.15       49.6%         0.33       49.5%  0.54 / -0.06
 *   COMPOSITE(셋 합산)    2.99       50.0%         1.17       49.4%  0.90 / 1.43
 *
 * 시장 성분을 제거하는 순간 전부 무너진다. 즉 지표들이 잡아낸 것은 "지금 시장이 추세다"이고,
 * 28개 코인은 같은 시장을 공유하므로 독립 관측이 아니다(이 함정에 이미 세 번 빠졌다).
 * MACD는 셋 중 가장 약하다(t=0.33, 적중률 49.5%로 동전 던지기 이하).
 * "우리는 기술적 지표를 씁니다"는 방법론 진술이지 예측력의 증거가 아니다.
 *
 * 점 예측의 방향과 크기는 오직 아래 사후 drift에서 나온다. 하루하루 오르내리는 예측선을
 * 그리지 않는 이유도 같다 — 일간 방향을 맞힐 수 있다는 증거가 없으므로(적중률 49.8%),
 * 정직한 점 예측 경로는 단조로운 곡선이다.
 *
 * "일봉으로 보다가 특정 기간부터 주봉·월봉으로 바꾸면 지그재그가 된다"는 발상도 검정했다.
 * 그렇게 되지 않는다.
 *   1) drift는 봉 크기와 무관하다. mean(k일 로그수익률)/k = mean(일간 로그수익률)은 항등식이다.
 *      BTC 연 drift: 일봉 +35.32% · 주봉 +35.32% · 월봉 +34.63%(마지막 미완성 봉 차이).
 *   2) 실제로 30일까지 일봉, 180일까지 주봉, 이후 월봉으로 경로를 만들면 364걸음 중 하락은
 *      단 1번이고, 그것도 전환 지점(d=180 -> 181)에서만 생긴다. 지그재그가 아니라 추정기를
 *      바꾼 자리의 인위적 계단이다.
 *   3) 봉이 굵어지면 관측 수가 줄어(3248 -> 108) 표준오차가 커지고 사후 가중치가
 *      0.754 -> 0.721로 떨어진다. 즉 경로가 더 "평평"해지지 더 흔들리지 않는다.
 *   4) 분산비는 7일 봉에서 1.00~1.04로 1에 가깝다(허스트 ~0.5와 일치). 집계는 새 정보를
 *      만들지 않는다. 다만 30일 봉에서는 1.18~1.39로 다소 크게 나오는데, 커버리지 백테스트
 *      (h=30/90에서 50.3~54.3%)에서 우리 밴드가 이미 명목치 이상이므로 조정하지 않는다.
 *
 * 지평에 따라 실제로 달라지는 것은 방향이 아니라 변동성이며, 그것은 아래 sigmaAt(h)가
 * 이미 반영한다. 상세 페이지의 일/주/월 뷰도 각 행이 자기 지평의 변동성을 쓴다.
 *
 * ── "과거 데이터로 더 훈련시키면 지그재그도 나오고 맞기도 할 것" ──────
 * 실제로 만들어 검정했다. 특성 21개(수익률 시차 1~5, SMA 20/50/100/200 거리, RSI,
 * MACD와 히스토그램, %B, 변동성비 2종, 거래량비, 요일 더미), 학습 표본 45,279개,
 * 코인 30종, 릿지 회귀. 표준화조차 학습 구간 통계만 사용해 미래 정보 누출을 차단했다.
 *
 *   정규화       인샘플 적중률  인샘플 R^2   아웃오브샘플 적중률  아웃오브샘플 R^2
 *   lambda=1e-6     51.25%      +0.0074          49.99%          -0.0029
 *   lambda=1e+1     51.24%      +0.0074          50.03%          -0.0028
 *
 * 지그재그는 실제로 나온다(모델을 30일 반복 적용하면 경로가 요동친다). 그러나 맞지 않는다.
 * 코인별로 마지막 400일을 떼어 학습/평가하면(22개 코인):
 *   평균 아웃오브샘플 적중률 48.79% (동전 던지기보다 나쁘다, t = -2.25)
 *   50%를 넘긴 코인 7/22 · 전략 수익 평균 -30.1% · 일간 수익 |t|>2인 코인 0/22
 *
 * 함정 주의: BTC 하나만 보면 신호 추종 수익 +44.0%, 매수보유 -47.3%로 대박처럼 보인다.
 * 그러나 (1) BTC의 적중률은 50.38%(t=0.15)로 무의미하고 (2) 22개 평균은 -30.1%이며
 * (3) 평가 구간이 하락장이라 동전 던지기라도 숏이 조금 더 많으면 매수보유를 이긴다.
 * 코인 하나·기간 하나의 백테스트가 좋아 보이는 것은 실력이 아니라 표본 추출이다.
 *
 * 결론: 훈련을 늘리면 인샘플 적합(51.25%)만 좋아지고 일반화(48.79%)는 오히려 나빠진다.
 * 지그재그하는 예측선은 만들 수 있으나, 그 굴곡은 잡음을 학습한 결과다.
 *
 * "48.79%면 유의하게 50% 아래이니 신호를 뒤집으면 되지 않느냐"도 검정했다. 되지 않는다.
 * 겹치지 않는 4개 구간으로 walk-forward(구간마다 그 이전 데이터로만 학습, 17개 코인):
 *   2023-10~2024-06  50.71% (t=+0.92)
 *   2024-06~2025-02  49.27% (t=-0.95)
 *   2025-02~2025-11  49.27% (t=-0.95)
 *   2025-11~2026-07  49.02% (t=-1.28)
 * 부호가 구간마다 뒤집히고 어느 구간도 |t|>2가 아니다(평균 49.57%, 구간 간 sd 0.77%p).
 * 즉 48.79%는 한 표본이었을 뿐 반전 신호가 아니다. 한 구간에서 졌다고 부호를 뒤집는 것은
 * 데이터 마이닝이다 — 진짜 반전 우위라면 모든 구간에서 져야 한다.
 *
 * ── 아웃오브샘플 적중률 종합 (유일하게 의미 있는 지표) ──────────
 *   기술적 합의(Trend/BB/RSI/ATR)        49.8%
 *   MA + RSI + MACD (경쟁사 공개 방식)   49.4%
 *   21특성 ML (릿지, 45,279 표본)        48.79%   <- 인샘플은 가장 높고(51.25%) OOS는 가장 낮다
 * 인샘플 우위와 아웃오브샘플 열위가 함께 나타나는 것이 과적합의 정의다.
 * "가장 정교한 모델"이 "가장 나쁜 예측기"인 이유가 여기 있다.
 *
