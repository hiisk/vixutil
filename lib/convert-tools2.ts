...
export const CONVERT_TOOLS2: ConvertTool[] = [
  ...
  {
    slug: 'mil-mm', title: 'mil ↔ mm', desc: '밀(thou)과 밀리미터를 서로 바꿉니다', icon: '📏',
    category: '길이', from: 'mil', to: 'mm', factor: 0.0254, digits: 4, quick: [1, 5, 10, 40, 100, 250],
    metaTitle: 'mil ↔ mm 변환 - 밀을 밀리미터로',
    long: '1mil은 1인치의 1000분의 1, 곧 0.0254mm입니다. 회로기판 배선 폭, 필름과 비닐 두께, 도장 두께를 미국식 도면에서 이 단위로 적습니다.',
    note: 'thou라고도 부르는 같은 단위입니다. 밀리미터(mm)와 이름이 비슷해 헷갈리기 쉬운데, 1mil은 1mm의 40분의 1도 안 됩니다.',
  },
  {
    slug: 'angstrom-nm', title: '옹스트롬 ↔ 나노미터', desc: '옹스트롬과 나노미터를 서로 바꿉니다', icon: '🔬',
    category: '길이', from: 'Å', to: 'nm', factor: 0.1, digits: 4, quick: [1, 5, 10, 50, 100, 500],
    metaTitle: '옹스트롬 ↔ 나노미터 변환 - 원자 크기 단위',
    long: '1옹스트롬은 0.1나노미터입니다. 원자의 지름과 화학 결합 길이가 대체로 1~3옹스트롬이라 결정학과 분광학에서 오래 쓰였습니다.',
    note: 'SI 단위가 아니라서 논문은 나노미터나 피코미터로 옮겨 적기를 권합니다. 값을 읽을 때 어느 쪽인지 확인하세요.',
  },
  {
    slug: 'pennyweight-g', title: '페니웨이트 ↔ g', desc: '금은 거래에 쓰는 페니웨이트를 그램으로 바꿉니다', icon: '⚖️',
    category: '무게', from: 'dwt', to: 'g', factor: 1.55517384, digits: 4, quick: [1, 2, 5, 10, 20, 50],
    metaTitle: '페니웨이트 ↔ 그램 변환 - 귀금속 무게 단위',
    long: '1페니웨이트(dwt)는 1.55517384g이고, 20페니웨이트가 1트로이온스입니다. 북미 귀금속 거래와 치과용 합금에서 아직 쓰입니다.',
    note: '보통 온스(28.35g)가 아니라 트로이온스(31.1035g)를 기준으로 하는 단위입니다. 둘을 섞으면 값이 10% 가까이 어긋납니다.',
  },
  {
    slug: 'tola-g', title: '톨라 ↔ g', desc: '남아시아 금 거래의 톨라를 그램으로 바꿉니다', icon: '🪙',
    category: '무게', from: 'tola', to: 'g', factor: 11.6638038, digits: 4, quick: [1, 2, 5, 10, 20, 50],
    metaTitle: '톨라 ↔ 그램 변환 - 인도·파키스탄 금 단위',
    long: '1톨라는 11.6638038g으로, 옛 인도 은화 한 닢의 무게에서 왔습니다. 인도·파키스탄·방글라데시의 금은방에서 아직 이 단위로 값을 매깁니다.',
  ...
  },
  {
    ...
    note: '미국식 질은 118.29mL로 영국식과 다릅니다. 옛 요리책을 볼 때 어느 나라 책인지 먼저 확인하세요.',
  },
  {
    slug: 'impgallon-l', title: '영국 갤런 ↔ L', desc: '영국식 갤런을 리터로 바꿉니다', icon: '⛽',
    category: '부피', from: 'gal', to: 'L', factor: 4.54609, digits: 4, quick: [1, 2, 5, 10, 20, 50],
    metaTitle: '영국 갤런 ↔ 리터 변환 - 미국 갤런과 다르다',
    long: '영국식 1갤런은 정확히 4.54609L입니다. 영국·아일랜드·카리브해 일부에서 쓰던 값으로, 미국식 갤런(3.785L)보다 20% 더 큽니다.',
    note: '연비를 견줄 때 가장 많이 틀리는 자리입니다. 영국식 갤런 기준 mpg는 미국식보다 항상 커 보이지만 실제 연비가 좋은 것은 아닙니다.',
  },
  {
    slug: 'rood-m2', title: '루드 ↔ ㎡', desc: '영국식 넓이 단위 루드를 제곱미터로 바꿉니다', icon: '🌾',
    category: '넓이', from: 'rood', to: 'm²', factor: 1011.7141056, digits: 3, quick: [1, 2, 4, 5, 10, 20],
    metaTitle: '루드 ↔ 제곱미터 변환 - 에이커의 4분의 1',
    long: '1루드는 1011.7141056㎡로 1에이커의 정확히 4분의 1입니다. 영국 옛 토지 문서와 농지 기록에 에이커·루드·퍼치가 나란히 적혀 있습니다.',
    note: '십자가를 뜻하는 같은 낱말이 따로 있어 옛 문서에서 헷갈립니다. 넓이 문맥일 때만 이 값입니다.',
  },
  {
    slug: 'pace-mile-kmh', title: '페이스(분/마일) ↔ km/h', desc: '마일당 페이스를 시속으로 바꿉니다', icon: '🏃',
    category: '속도', from: '분/mile', to: 'km/h', factor: 96.56064, reciprocal: true, digits: 3, quick: [6, 7, 8, 9, 10, 12],
    metaTitle: '페이스(분/마일) ↔ km/h 변환 - 달리기 속도',
    long: '마일당 몇 분으로 달리는지를 시속으로 바꿉니다. 1마일이 1.609344km이므로 96.56을 페이스로 나누면 시속이 됩니다. 페이스 8분이면 시속 12.07km입니다.',
    note: '나누기라서 방향이 뒤집힙니다. 페이스 숫자가 작을수록 빠르고 시속 숫자가 클수록 빠릅니다.',
  },
  {
    slug: 'tib-tb', title: 'TiB ↔ TB', desc: '2의 거듭제곱 테비바이트와 10의 거듭제곱 테라바이트', icon: '💾',
    category: '데이터', from: 'TiB', to: 'TB', factor: 1.099511627776, digits: 4, quick: [1, 2, 4, 8, 16, 32],
    metaTitle: 'TiB ↔ TB 변환 - 하드디스크 용량이 줄어 보이는 이유',
    long: '1TiB은 2의 40제곱 바이트이고 1TB은 10의 12제곱 바이트라 1TiB이 약 1.0995TB입니다. 4TB 디스크가 3.64TiB으로 보이는 것이 이 차이 때문입니다.',
    note: '용량이 사라진 것이 아니라 세는 단위가 다른 것입니다. 윈도우는 TiB을 TB이라 적고, macOS와 제조사는 TB을 씁니다.',
  },
  {
    slug: 'btu-wh', title: 'BTU ↔ Wh', desc: '열량 단위 BTU를 와트시로 바꿉니다', icon: '🔥',
    category: '에너지', from: 'BTU', to: 'Wh', factor: 0.29307107, digits: 4, quick: [1, 10, 100, 500, 1000, 3412],
    metaTitle: 'BTU ↔ 와트시 변환 - 냉난방 용량 단위',
    long: '1BTU는 약 0.293071Wh입니다. 에어컨과 보일러의 용량을 북미에서 BTU로 적으므로, 전력 단위로 바꿔야 소비전력과 견줄 수 있습니다.',
    note: '에어컨의 "BTU"는 대개 시간당 BTU(BTU/h)를 줄여 부르는 말입니다. 12,000BTU/h가 1냉동톤입니다.',
  },
];
