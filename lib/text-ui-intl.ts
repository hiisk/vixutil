...
export const CLEAN_UI: Record<TextLang, {
  ...
  'zh-hans': {
    labels: ['删掉看不见的字符', '怪空格换成普通空格', '合并重复的空格', '去掉每行首尾空白', '减少连续空行', '接回断掉的行', '弯引号换成直引号', '去掉HTML标签'],
    hints: ['零宽空格、BOM之类 — 它们撑大字数、还让搜索失灵', '看着像空格其实不是的字符（NBSP之类）', '两个以上的空格并成一个', '', '三行以上的空行并成一行', '从PDF复制会在句子中间断行', '“ ” ‘ ’ → " \'', '去掉<p>这类标签'],
    inputLabel: '把要整理的文字贴进来',
    placeholder: '从PDF、网页或Word复制的文字，原样贴进来就行',
    whatTitle: '要整理什么',
    nothing: '没有要修的地方 — 这段文字已经很干净',
    shrunk: n => `删掉了${n}个字符`,
    cleaned: '整理完成',
    outputLabel: '整理后的文字',
  },
  'zh-hant': {
    labels: ['刪掉看不見的字元', '怪空格換成普通空格', '合併重複的空格', '去掉每行首尾空白', '減少連續空行', '接回斷掉的行', '彎引號換成直引號', '去掉HTML標籤'],
    hints: ['零寬空格、BOM之類 — 它們撐大字數、還讓搜尋失靈', '看著像空格其實不是的字元（NBSP之類）', '兩個以上的空格併成一個', '', '三行以上的空行併成一行', '從PDF複製會在句子中間斷行', '“ ” ‘ ’ → " \'', '去掉<p>這類標籤'],
    inputLabel: '把要整理的文字貼進來',
    placeholder: '從PDF、網頁或Word複製的文字，原樣貼進來就行',
    whatTitle: '要整理什麼',
    nothing: '沒有要修的地方 — 這段文字已經很乾淨',
    shrunk: n => `刪掉了${n}個字元`,
    cleaned: '整理完成',
    outputLabel: '整理後的文字',
  },
...
export const DEDUPE_UI: Record<TextLang, {
  ...
  'zh-hans': {
    inputLabel: '把列表贴进来（一行一个）',
    placeholder: '张三\n李四\n张三\n王五',
    dedupe: '删除重复行',
    ignoreSpace: '空白不同也算同一行',
    ignoreSpaceHint: '把「张三」和「张三 」当成一个',
    ignoreCase: '大小写不同也算同一行',
    ignoreCaseHint: '把Apple和apple当成一个',
    removeBlank: '删掉空行',
    numbered: '加上行号',
    numberedHint: '在前面加1. 2. 3.',
    sortTitle: '排序',
    sortModes: ['原来的顺序', 'A → Z', 'Z → A'],
    totalLines: '原有行数',
    keptLines: '留下行数',
    removedLines: '删掉行数',
    outputLabel: '整理后的列表',
  ...
  'zh-hant': {
    inputLabel: '把清單貼進來（一行一個）',
    placeholder: '張三\n李四\n張三\n王五',
    dedupe: '刪除重複行',
    ignoreSpace: '空白不同也算同一行',
    ignoreSpaceHint: '把「張三」和「張三 」當成一個',
    ignoreCase: '大小寫不同也算同一行',
    ignoreCaseHint: '把Apple和apple當成一個',
    removeBlank: '刪掉空行',
    numbered: '加上行號',
    numberedHint: '在前面加1. 2. 3.',
    sortTitle: '排序',
    sortModes: ['原來的順序', 'A → Z', 'Z → A'],
    totalLines: '原有行數',
    keptLines: '留下行數',
    removedLines: '刪掉行數',
    outputLabel: '整理後的清單',
...
export const CASE_UI: Record<TextLang, {
  ...
  'zh-hans': {
    labels: ['UPPERCASE', 'lowercase', 'Title Case', 'Sentence case', 'camelCase', 'PascalCase', 'snake_case', 'kebab-case', 'CONSTANT_CASE', 'tOGGLE cASE'],
    hints: ['', '', '用在标题和名字上', '', '变量名', '类名和组件名', '数据库列名、Python', '网址、CSS类名', '环境变量、常量', ''],
    inputLabel: '输入你的文字',
    placeholder: '例）hello world example',
    noteTitle: '没有大小写的文字会怎样',
    note: '汉字、韩文、日文假名没有大小写之分，所以那些字原样保留。中英混排的文字里，只有拉丁字母会变。',
  },
  'zh-hant': {
    labels: ['UPPERCASE', 'lowercase', 'Title Case', 'Sentence case', 'camelCase', 'PascalCase', 'snake_case', 'kebab-case', 'CONSTANT_CASE', 'tOGGLE cASE'],
    hints: ['', '', '用在標題和名字上', '', '變數名', '類別名和元件名', '資料庫欄位名、Python', '網址、CSS類別名', '環境變數、常數', ''],
    inputLabel: '輸入你的文字',
    placeholder: '例）hello world example',
    noteTitle: '沒有大小寫的文字會怎樣',
    note: '漢字、韓文、日文假名沒有大小寫之分，所以那些字原樣保留。中英混排的文字裡，只有拉丁字母會變。',
  },
};
...
export const REPLACE_UI: Record<TextLang, {
  ...
  'zh-hans': {
    sourceLabel: '原文',
    sourcePlaceholder: '把含有要改内容的文字贴进来',
    findLabel: '查找',
    findPlaceholder: '要替换的词',
    toLabel: '替换成',
    toPlaceholder: '新的词（留空就是删掉）',
    caseSensitive: '区分大小写',
    caseSensitiveHint: '关掉的话Apple和apple都会被找到',
    regex: '用正则表达式查找',
    regexHint: '可以用\\d+这样的模式',
    escapeNoteBefore: '在查找框里填 ',
    escapeNoteMid: ' 可以找换行，',
    escapeNoteAfter: ' 可以找制表符。',
    regexError: msg => `正则表达式错误: ${msg}`,
    willChange: n => `会改动${n}处`,
    noMatch: '没有找到',
    outputLabel: '替换后的结果',
  },
  'zh-hant': {
    sourceLabel: '原文',
    sourcePlaceholder: '把含有要改內容的文字貼進來',
    findLabel: '尋找',
    findPlaceholder: '要取代的詞',
    toLabel: '取代成',
    toPlaceholder: '新的詞（留空就是刪掉）',
    caseSensitive: '區分大小寫',
    caseSensitiveHint: '關掉的話Apple和apple都會被找到',
    regex: '用正規表示式尋找',
    regexHint: '可以用\\d+這樣的模式',
    escapeNoteBefore: '在尋找框裡填 ',
    escapeNoteMid: ' 可以找換行，',
    escapeNoteAfter: ' 可以找定位字元。',
    regexError: msg => `正規表示式錯誤: ${msg}`,
    willChange: n => `會改動${n}處`,
    noMatch: '沒有找到',
    outputLabel: '取代後的結果',
  },
};
...
export const MANUSCRIPT_UI: Record<TextLang, {
  ...
  'zh-hans': {
    inputLabel: '把文字贴进来',
    placeholder: '作文、申请书或者稿子，原样贴进来就行',
    withSpaces: '含空格',
    withoutSpaces: '不含空格',
    sheets200: '页数（200字）',
    sheets400: '页数（400字）',
    ...
    targetTitle: '你的上限',
    countingWith: '按含空格数',
    countingWithout: '按不含空格数',
    charSuffix: n => `${n}字`,
    emptyHint: '填进文字就会算出还剩多少',
    used: (used, target, left) => `${used} / ${target} · 还能写${left}`,
    over: (used, target, over) => `${used} / ${target} · 超了${over}`,
    noteTitle: '该按哪种数法',
    notes: ['· 申请表格几乎都是含空格数的。要求里没写清楚的话，按含空格来对比较保险。', '· 作文和文章一般看词数，表格和输入框限制的是字符数。', '· 输入框有字数限制的网站，几乎都把空格算进去。'],
  },
  'zh-hant': {
    inputLabel: '把文字貼進來',
    placeholder: '作文、申請書或者稿子，原樣貼進來就行',
    withSpaces: '含空格',
    withoutSpaces: '不含空格',
    sheets200: '頁數（200字）',
    sheets400: '頁數（400字）',
    ...
    targetTitle: '你的上限',
    countingWith: '按含空格數',
    countingWithout: '按不含空格數',
    charSuffix: n => `${n}字`,
    emptyHint: '填進文字就會算出還剩多少',
    used: (used, target, left) => `${used} / ${target} · 還能寫${left}`,
    over: (used, target, over) => `${used} / ${target} · 超了${over}`,
    noteTitle: '該按哪種數法',
    notes: ['· 申請表格幾乎都是含空格數的。要求裡沒寫清楚的話，按含空格來對比較保險。', '· 作文和文章一般看詞數，表格和輸入框限制的是字元數。', '· 輸入框有字數限制的網站，幾乎都把空格算進去。'],
  },
};
...
export const LOREM_UI: Record<TextLang, {
  ...
  'zh-hans': {
    ...
    charUnit: '',
    regenerate: '🔄 换一份文字',
    sameSeed: seed => `设定不变，出来的也不变（现在是第${seed}份）`,
    charCount: '字数',
    paragraphs: '段落',
    words: '词数',
    outputLabel: '生成的文字',
    noteTitle: '为什么要有中日韩这一项',
    note: '拉丁文的Lorem ipsum字形窄、空格多，中日韩文字比它宽得多。拿拉丁填充文排出来的版面，换成真的中文内容行数就变多、版面就散了。要上线的是中文界面，就用中日韩填充文来试。',
  },
  'zh-hant': {
    noLimit: '不限',
    langs: ['中日韓填充文', '拉丁文（Lorem ipsum）'],
    paragraphCount: '段落數',
    paragraphUnit: '',
    sentenceCount: '每段句數',
    sentenceUnit: '',
    ...
    charUnit: '',
    regenerate: '🔄 換一份文字',
    sameSeed: seed => `設定不變，出來的也不變（現在是第${seed}份）`,
    charCount: '字數',
    paragraphs: '段落',
    words: '詞數',
    outputLabel: '產生的文字',
    noteTitle: '為什麼要有中日韓這一項',
    note: '拉丁文的Lorem ipsum字形窄、空格多，中日韓文字比它寬得多。拿拉丁填充文排出來的版面，換成真的中文內容行數就變多、版面就散了。要上線的是中文介面，就用中日韓填充文來試。',
  },
};
...
export const COPY_PICKER_UI: Record<TextLang, {
  ...
  'zh-hans': {
    copied: '已复制',
    searchPlaceholder: '按名字搜 — 箭头、平方米、爱心…',
    recentTitle: '最近用过的',
    foundCount: n => `${n}个结果`,
    notFound: '没有对上的符号。换个名字再搜搜看。',
  },
  'zh-hant': {
    copied: '已複製',
    searchPlaceholder: '按名字搜 — 箭頭、平方公尺、愛心…',
    recentTitle: '最近用過的',
    foundCount: n => `${n}個結果`,
    notFound: '沒有對上的符號。換個名字再搜搜看。',
  },
};
...
export const SYMBOL_TOOL_UI: Record<TextLang, { specialHint: string; emoticonHint: string }> = {
  ...
  'zh-hans': {
    specialHint: '点一下就复制到剪贴板。贴过去的地方字体里没有这个字形的话，会显示成方框（□），那就换个符号或者换字体。',
    emoticonHint: '这些是用字符搭的，不是图片。在不收emoji的地方也能用，比如用户名和状态栏，而且在哪台设备上看都一样。',
  },
  'zh-hant': {
    specialHint: '點一下就複製到剪貼簿。貼過去的地方字體裡沒有這個字形的話，會顯示成方框（□），那就換個符號或者換字體。',
    emoticonHint: '這些是用字元搭的，不是圖片。在不收emoji的地方也能用，比如使用者名稱和狀態欄，而且在哪台裝置上看都一樣。',
  },
};
