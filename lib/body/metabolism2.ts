...
export const METABOLISM2_TOOLS: FormulaTool[] = [
  ...
  {
    slug: 'calories-per-meal',
    icon: '🍽️',
    category: '대사·칼로리',
    fields: [
      { key: 'cal', term: 'calories', unit: 'kcal', def: 2000, min: 0 },
      { key: 'meals', term: 'mealsPerDay', unit: 'times', def: 3, min: 0 },
    ],
    formula: '{foodKcal} = {calories} ÷ {mealsPerDay}',
    compute: v => [
      { term: 'foodKcal', unit: 'kcal', value: Math.round(ratio(v.cal, v.meals)), digits: 0, primary: true },
      { term: 'proteinPerMeal', unit: 'gram', value: round(ratio(v.cal * 0.2 / 4, v.meals), 1), digits: 1 },
    ],
    ko: { title: '한 끼 칼로리 배분', desc: '하루 목표 열량을 끼니 수로 나눠 한 끼에 얼마를 먹을지 정합니다.',
      long: '하루 열량을 끼니 수로 나누기만 하면 됩니다. 단백질 몫은 열량의 20%를 단백질로 잡고 4kcal/g으로 되돌린 값입니다.',
      note: '간식을 끼니 수에 넣지 않으면 실제로는 더 먹게 됩니다. 하루에 몇 번 입에 넣는지로 세는 편이 맞습니다.' },
    en: { title: 'Calories per Meal', desc: 'Split a daily calorie target across the number of meals you actually eat.',
      long: 'Divide the daily target by how many meals you eat. The protein figure assumes a fifth of the calories come from protein and converts back at 4 kcal per gram.',
      note: 'Leave snacks out of the meal count and you will eat more than the plan says. Count every time you actually eat.' },
  },
  {
    slug: 'food-calorie-share',
    icon: '🥧',
    category: '대사·칼로리',
    fields: [
      { key: 'food', term: 'foodKcal', unit: 'kcal', def: 500, min: 0 },
      { key: 'cal', term: 'calories', unit: 'kcal', def: 2000, min: 0 },
    ],
    formula: '{percent} = {foodKcal} ÷ {calories} × 100',
    compute: v => [
      { term: 'percent', unit: 'percent', value: round(ratio(v.food, v.cal) * 100, 1), digits: 1, primary: true },
      { term: 'remaining', unit: 'kcal', value: Math.round(v.cal - v.food), digits: 0 },
    ],
    ko: { title: '이 음식이 하루의 몇 %인가', desc: '한 가지 음식의 열량이 하루 목표에서 차지하는 몫을 봅니다.',
      long: '음식 열량을 하루 열량으로 나누고 100을 곱합니다. 포장지의 칼로리는 대개 100g 기준이라, 실제로 먹는 양으로 고쳐 넣어야 합니다.',
      note: '남는 열량이 음수로 나오면 그 한 끼로 하루치를 이미 넘긴 것입니다.' },
    en: { title: 'Share of Your Daily Calories', desc: 'See what slice of the day one food takes out of your calorie target.',
      long: 'Divide the food energy by the daily target and multiply by 100. Labels usually quote per 100 g, so scale it to the portion you actually eat before entering it.',
      note: 'A negative remainder means that single item has already used up the whole day.' },
  },
];
