...
const EXTRACT: Pattern[] = [
  ...
  /* ───────── 늘린 것 ───────── */
  p('ipv6-compressed', 'validate', '^([0-9a-fA-F]{1,4}(:[0-9a-fA-F]{1,4})*)?::([0-9a-fA-F]{1,4}(:[0-9a-fA-F]{1,4})*)?$',
    ['::1', 'fe80::', '2001:db8::1'], ['192.168.0.1', '2001:db8:0:0:0:0:0:1', 'hello']),
  p('korean-biz-no', 'validate', '^\\d{3}-\\d{2}-\\d{5}$',
    ['123-45-67890', '000-11-22222'], ['123-4-67890', '12345678901', '123-45-6789']),
  p('mime-type', 'validate', '^[a-z]+\\/[a-zA-Z0-9][a-zA-Z0-9.+-]*$',
    ['text/html', 'image/svg+xml', 'application/vnd.ms-excel'], ['text', 'Text/HTML', 'text//html']),
  p('data-uri', 'validate', '^data:[a-z]+\\/[a-z0-9.+-]+;base64,[A-Za-z0-9+/]+=*$',
    ['data:image/png;base64,iVBORw0KGgo=', 'data:text/plain;base64,aGk='], ['data:image/png,abc', 'https://a.com/b.png']),
  p('cron-expression', 'validate', '^(\\S+\\s+){4}\\S+$',
    ['0 3 * * 1', '*/5 * * * *', '0 0 1 1 *'], ['0 3 * *', '0 3 * * 1 2 3', 'daily']),
  p('windows-drive', 'validate', '^[A-Za-z]:\\\\.*$',
    ['C:\\Users', 'D:\\', 'z:\\tmp\\a'], ['/home/me', 'C:/Users', 'CD:\\x']),
  p('semver-range', 'validate', '^[\\^~]?\\d+\\.\\d+\\.\\d+$',
    ['^1.2.3', '~0.4.0', '2.0.0'], ['^1.2', 'v1.2.3', '>=1.2.3']),
  p('ipv4-private', 'validate', '^(10\\.\\d{1,3}\\.\\d{1,3}\\.\\d{1,3}|192\\.168\\.\\d{1,3}\\.\\d{1,3}|172\\.(1[6-9]|2\\d|3[01])\\.\\d{1,3}\\.\\d{1,3})$',
    ['10.0.0.1', '192.168.1.1', '172.20.5.4'], ['8.8.8.8', '172.15.0.1', '172.32.0.1']),
  p('pascal-case', 'validate', '^[A-Z][a-zA-Z0-9]*$',
    ['PascalCase', 'Http2Server', 'A'], ['pascalCase', 'Pascal_Case', '2Pascal']),
  p('markdown-image', 'extract', '!\\[([^\\]]*)\\]\\(([^)]+)\\)',
    ['![고양이](cat.png)', 'before ![](x.jpg) after'], ['[link](a.com)', 'no image here']),
  p('markdown-list', 'extract', '^[-*+]\\s+(.+)$',
    ['- 첫째', '* 둘째', '+ 셋째'], ['1. 첫째', '-없는공백', 'plain']),
  p('sql-comment', 'extract', '--[^\\n]*',
    ['SELECT 1 -- 주석', '-- 줄 전체'], ['SELECT 1', '/* block */']),
  p('env-var', 'extract', '\\$\\{?([A-Z_][A-Z0-9_]*)\\}?',
    ['$HOME', '${DATABASE_URL}', 'echo $PATH'], ['$lower', 'PATH', '${1}']),
  p('youtube-id', 'extract', '[A-Za-z0-9_-]{11}',
    ['dQw4w9WgXcQ', 'youtu.be/aBcDeFgHiJk'], ['short', 'ab-cd']),
];
