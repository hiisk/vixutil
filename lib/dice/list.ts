/**
 * 주사위 합 188가지 — 몇 개를 굴려 얼마가 나오는가.
 *
 * 자료라고 할 것이 거의 없다. 주사위 개수와 합, 두 숫자뿐이다. 경우의 수도
 * 확률도 순위도 전부 계산한다(facts.ts). 확률표를 손으로 옮겨 적으면 188줄에
 ...
 * 한 개부터 여덟 개까지.
 *
 * 일곱·여덟 개는 탁상 놀이에서 실제로 굴리는 수다. 경우의 수는 한 개짜리
 * 분포에 하나씩 겹쳐 구하므로(facts.ts) 개수가 늘어도 셈이 무거워지지 않는다 —
 * 6^8=1,679,616가지를 하나씩 세는 것이 아니다.
 */
export const DICE_COUNTS = [1, 2, 3, 4, 5, 6, 7, 8];
