...
/* 생성됨: scripts가 아니라 접기 이행 — 원본은 옛 app/(zh-hant)/zh-hant/rate/page.tsx.
   아홉 언어 라우트 파일을 이 모듈 하나로 접었다. 목록은 lib/fold/registry.ts */
export function build(lang: FoldLang) {
  /* 화면은 components/FormulaHub.tsx 하나를 열 언어가 같이 쓴다 */
  const meta = sectionMeta(RATE_SECTION, lang);
  ...
  const metadata: Metadata = withCard({
    ...
    alternates: {
      canonical: localeHref(lang, '/rate'),
      languages: sectionAlternates('rate', undefined, RATE_LANGS),
    },
  });

  function Page() {
    return <FormulaHub lang={lang} section={RATE_SECTION} />;
  }
