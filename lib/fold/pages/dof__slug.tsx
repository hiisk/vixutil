...
import DofPage from '@/components/dof/DofPage';
import { cellOf } from '@/lib/dof/list';
import { detailMetadata, dofParams } from '@/lib/dof/route';
import type { FoldLang } from '../lang';
import { DATA_KEY } from '../lang';

/* 아홉 언어가 이 모듈 하나를 같이 쓴다 — 낱장은 언어마다 남아 있는
   app/(xx)/xx/dof/[slug]/page.tsx 껍데기가 이것을 부른다(요청 때 그린다) */
export function build(lang: FoldLang) {
  ...
  async function Page({ params }: { params: Promise<{ slug: string }> }) {
    ...
    return <DofPage slug={slug} lang={DATA_KEY[lang]} />;
  }
   ...
   */
  const generateStaticParams = () => dofParams();
