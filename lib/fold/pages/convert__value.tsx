...
import ValueLeaf from '@/components/ValueLeaf';
 ...
 */
export function build(lang: ConvertLang) {
  ...
  async function Page({ params }: { params: Promise<{ b: string; slug: string }> }) {
    const { b, slug } = await params;
    const hit = resolve(b, slug);
    if (!hit) notFound();
    return <ValueLeaf slug={hit.tool.slug} value={hit.v} lang={lang} />;
  }
