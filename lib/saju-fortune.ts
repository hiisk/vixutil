...
export function analyzeFortune(
  ...
  /* ─── 3-2. 승진운 ──────────────────────────────────────────────────────────
   * 취업(career)·이직(change)과 짚는 자리가 다르다.
   *  · 정관(正官)은 조직 안의 직급과 규범이라 승진의 첫 근거다.
   *  · 관인상생(官印相生)은 정관이 정인을 낳는 구조로, 스스로 밀어붙인 것이
   *    아니라 조직이 자리를 내주는 승진(발령·임명)의 전형으로 친다.
   *  · 상관견관(傷官見官)은 상관이 정관을 쳐서 직장과 명예가 흔들린다 —
   *    淵海子平·命理正宗의 "傷官見官 爲禍百端".
   * 점수 식은 lib/saju-fortune-facts.ts에 한 벌만 있다.
   */
  const promotionScore = F.scores.promotion;
  const hasJeonggwan = allSS.includes('정관');

  const promotion: DomainFortune = {
    id: 'promotion', emoji: '🏅', title: '승진운',
    score: promotionScore, grade: GRADES[promotionScore], colorKey: 'teal',
    intro: `승진은 직업운과 보는 자리가 다릅니다. 어떤 일이 맞는가는 식상·재성이 정하지만, 조직 안에서 자리가 올라가는가는 관성(官星) 가운데 정관(正官)이 정합니다. 정관은 직급·규범·책임의 별이기 때문입니다. 여기에 인성(印星)이 더해져 관성이 인성을 낳는 관인상생(官印相生)이 되면, 스스로 밀어붙여 얻는 자리가 아니라 조직이 내주는 자리 — 발령과 임명 — 의 구조가 됩니다. 반대로 상관(傷官)이 정관을 치는 상관견관(傷官見官)은 쌓아 온 명예가 한 번에 무너지는 자리라 예로부터 크게 경계했습니다.`,
    summary: promotionScore >= 4
      ? '조직 안에서 자리가 열리는 구조입니다. 때가 오면 밀리지 않고 올라갑니다.'
      : promotionScore === 3
      ? '승진이 막히지는 않으나 저절로 오지도 않습니다. 근거를 남기는 쪽이 앞섭니다.'
      : '조직의 사다리보다 실력과 이름값으로 자리를 만드는 편이 빠른 사주입니다.',
    points: [
      hasJeonggwan
        ? '정관(正官)이 있어 조직 안의 직급이 실제로 열리는 길입니다. 튀는 성과 한 번보다 쌓인 신뢰가 자리를 만듭니다.'
        : allSS.includes('편관')
        ? '편관(偏官)만 있어 연차로 올라가기보다 위기와 결단의 순간에 발탁되는 구조입니다. 평시보다 사고가 터졌을 때 값이 오릅니다.'
        : '관성(官星)이 없어 조직의 사다리와는 인연이 옅습니다. 직급보다 전문성이나 자기 사업으로 이름을 얻는 쪽이 맞습니다.',
      F.gwanInSangsaeng
        ? '관인상생(官印相生)이 섰습니다. 승진이 내 요구가 아니라 위의 발령으로 오는 전형적인 구조이며, 쌓아 둔 공부와 문서가 그때 값을 합니다.'
        : '관인상생이 서지 않아 자리는 기다린다고 오지 않습니다. 자격과 기록으로 근거를 만들어 두어야 이름이 올라갑니다.',
      F.sanggwanGyeonGwan
        ? '상관견관(傷官見官)이 있습니다. 바른말과 윗사람과의 마찰 한 번이 쌓아 온 자리를 하루에 지울 수 있습니다 — 평가철에는 특히 말을 아끼세요.'
        : '상관견관이 없어 윗선과 구조적으로 부딪히는 자리는 아닙니다. 쌓아 온 평판이 쉽게 깎이지 않습니다.',
      sc.인성 >= 2
        ? '인성(印星)이 두터워 문서·자격·학위가 그대로 지렛대가 됩니다. 자격을 모으는 노력이 가장 확실한 투자입니다.'
        : sc.인성 === 0
        ? '인성(印星)이 없어 일한 것에 비해 남는 문서가 얇습니다. 성과를 기록하고 증명해 두는 습관이 승진의 관건입니다.'
        : '인성(印星)이 하나여서 자격 하나가 오래 자신을 끌어 줍니다. 넓히기보다 그 하나를 깊게 파는 편이 낫습니다.',
    ],
    advice: F.gwanInSangsaeng
      ? '자리가 열리는 구조이니 조바심 낼 필요가 없습니다. 자격과 실적을 문서로 정리해 두고 발령의 때를 기다리세요.'
      : '기다리기만 하면 지나갑니다. 원하는 자리를 먼저 말하고, 그 자리에 필요한 자격을 눈에 보이게 갖추세요.',
  };
  ...
  const future: DomainFortune = {
    ...
    points: [
      ...
      // 60자로 자르면 낱말 가운데서 끊겨 "…성격입니…" 같은 문장이 나왔다.
      // 영어판(saju-fortune-intl.ts)처럼 첫 문장만 온전히 가져온다.
      `${dayStem.kor}(${dayStem.hanja}) 일간의 핵심 강점: ${dayStem.personality.split('다. ')[0]}다.`,
    ],
